...
const TARGET_ID = "DESIGN_INPUT_EXPRESSION_TARGET::CAPACITY_AUTHORITY_PARTIAL_WIRING::9.0";
const IMPLEMENTATION_REVISION = "3b662149d4647239f96acf309ee2e39ed4b41b4d";
...
const CANVAS_TRIM_MAXIMUM_WIRED_IDS = [
...
function main(): void {
  ...
  requireEqual(target.productWiring.wiredAxisCount, 57, "wired product axes");
  requireEqual(target.productWiring.remainingAxisCount, 8, "remaining product axes");
  ...
  const expectedWired = [...predecessorWired, ...CANVAS_TRIM_MAXIMUM_WIRED_IDS].sort();
  const targetWired = target.productWiring.wiredLimitIds as string[];
  requireEqual(new Set(targetWired).size, 57, "unique wired product axes");
  ...
  requireEqual(target.productWiring.remainingGroups, [
    { group: "geometry", axisCount: 8, status: "PRODUCT_RESERVATION_POINTS_PENDING" },
  ...
  requireEqual(target.productWiring.geometryReservationProofAxisCount, 2,
  ...
  requireEqual(primary.boundary.wiredProductAxisCount, 57, "primary wired axes");
  requireEqual(primary.boundary.remainingProductAxisCount, 8, "primary remaining axes");
  ...
  const report = {
    ...
    targetManifest: {
      path: ".scratch/renderweave-template-v1/design-input-expression/capacity-component-target-v9.json",
    ...
    boundary: {
      ...
      wiredProductAxisCount: 57,
      remainingProductAxisCount: 8,
  ...
  process.stdout.write(
    `DESIGN_INPUT_EXPRESSION independent replay: ${observedCases.size}/195 PASS, 57/65 wired\n`,
  );
}
