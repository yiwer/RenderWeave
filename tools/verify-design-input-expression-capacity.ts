...
const TARGET_ID = "DESIGN_INPUT_EXPRESSION_TARGET::CAPACITY_AUTHORITY_PARTIAL_WIRING::13.0";
const IMPLEMENTATION_REVISION = "3f51f8203ad7669f66be67efac0a4e38d07a1958";
...
const FONT_SIZE_MINIMUM_WIRED_IDS = [
...
function main(): void {
  ...
  requireEqual(target.productWiring.wiredAxisCount, 61, "wired product axes");
  requireEqual(target.productWiring.remainingAxisCount, 4, "remaining product axes");
  ...
  requireEqual(predecessor.productWiring.geometryReservationProofAxisCount, 5,
    "predecessor Geometry reservation proof axis count");
  const expectedWired = [...predecessorWired, ...FONT_SIZE_MINIMUM_WIRED_IDS].sort();
  const targetWired = target.productWiring.wiredLimitIds as string[];
  requireEqual(new Set(targetWired).size, 61, "unique wired product axes");
  ...
  requireEqual(target.productWiring.remainingGroups, [
    { group: "geometry", axisCount: 4, status: "PRODUCT_RESERVATION_POINTS_PENDING" },
  ...
  requireEqual(target.productWiring.geometryReservationProofAxisCount, 6,
  ...
  requireEqual(primary.boundary.wiredProductAxisCount, 61, "primary wired axes");
  requireEqual(primary.boundary.remainingProductAxisCount, 4, "primary remaining axes");
  ...
  const report = {
    ...
    targetManifest: {
      path: ".scratch/renderweave-template-v1/design-input-expression/capacity-component-target-v13.json",
    ...
    boundary: {
      ...
      wiredProductAxisCount: 61,
      remainingProductAxisCount: 4,
  ...
  process.stdout.write(
    `DESIGN_INPUT_EXPRESSION independent replay: ${observedCases.size}/195 PASS, 61/65 wired\n`,
  );
}
