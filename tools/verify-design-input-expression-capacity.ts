...
const TARGET_ID = "DESIGN_INPUT_EXPRESSION_TARGET::CAPACITY_AUTHORITY_PARTIAL_WIRING::15.0";
const IMPLEMENTATION_REVISION = "93ad46fb39215e3f9e99e3649737e1efc425fcff";
...
const TRANSFORM_SCALE_WIRED_IDS = [
...
function main(): void {
  ...
  requireEqual(target.productWiring.wiredAxisCount, 63, "wired product axes");
  requireEqual(target.productWiring.remainingAxisCount, 2, "remaining product axes");
  ...
  requireEqual(predecessor.productWiring.geometryReservationProofAxisCount, 7,
    "predecessor Geometry reservation proof axis count");
  const expectedWired = [...predecessorWired, ...TRANSFORM_SCALE_WIRED_IDS].sort();
  const targetWired = target.productWiring.wiredLimitIds as string[];
  requireEqual(new Set(targetWired).size, 63, "unique wired product axes");
  ...
  requireEqual(target.productWiring.remainingGroups, [
    { group: "geometry", axisCount: 2, status: "PRODUCT_RESERVATION_POINTS_PENDING" },
  ...
  requireEqual(target.productWiring.geometryReservationProofAxisCount, 8,
  ...
  requireEqual(primary.boundary.wiredProductAxisCount, 63, "primary wired axes");
  requireEqual(primary.boundary.remainingProductAxisCount, 2, "primary remaining axes");
  ...
  const report = {
    ...
    targetManifest: {
      path: ".scratch/renderweave-template-v1/design-input-expression/capacity-component-target-v15.json",
    ...
    boundary: {
      ...
      wiredProductAxisCount: 63,
      remainingProductAxisCount: 2,
  ...
  process.stdout.write(
    `DESIGN_INPUT_EXPRESSION independent replay: ${observedCases.size}/195 PASS, 63/65 wired\n`,
  );
}
