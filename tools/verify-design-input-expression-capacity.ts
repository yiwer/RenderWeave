...
const TARGET_ID = "DESIGN_INPUT_EXPRESSION_TARGET::CAPACITY_AUTHORITY_PARTIAL_WIRING::14.0";
const IMPLEMENTATION_REVISION = "1f8fc3596f43d6844ed9cabde3ef23f9d4cd3a7b";
...
const FONT_SIZE_MAXIMUM_WIRED_IDS = [
...
function main(): void {
  ...
  requireEqual(target.productWiring.wiredAxisCount, 62, "wired product axes");
  requireEqual(target.productWiring.remainingAxisCount, 3, "remaining product axes");
  ...
  requireEqual(predecessor.productWiring.geometryReservationProofAxisCount, 6,
    "predecessor Geometry reservation proof axis count");
  const expectedWired = [...predecessorWired, ...FONT_SIZE_MAXIMUM_WIRED_IDS].sort();
  const targetWired = target.productWiring.wiredLimitIds as string[];
  requireEqual(new Set(targetWired).size, 62, "unique wired product axes");
  ...
  requireEqual(target.productWiring.remainingGroups, [
    { group: "geometry", axisCount: 3, status: "PRODUCT_RESERVATION_POINTS_PENDING" },
  ...
  requireEqual(target.productWiring.geometryReservationProofAxisCount, 7,
  ...
  requireEqual(primary.boundary.wiredProductAxisCount, 62, "primary wired axes");
  requireEqual(primary.boundary.remainingProductAxisCount, 3, "primary remaining axes");
  ...
  const report = {
    ...
    targetManifest: {
      path: ".scratch/renderweave-template-v1/design-input-expression/capacity-component-target-v14.json",
    ...
    boundary: {
      ...
      wiredProductAxisCount: 62,
      remainingProductAxisCount: 3,
  ...
  process.stdout.write(
    `DESIGN_INPUT_EXPRESSION independent replay: ${observedCases.size}/195 PASS, 62/65 wired\n`,
  );
}
