...
const TARGET_ID = "DESIGN_INPUT_EXPRESSION_TARGET::CAPACITY_AUTHORITY_PARTIAL_WIRING::10.0";
const IMPLEMENTATION_REVISION = "21ff1c45440c25c8d8e7f0edebd2d8b29d6d8902";
...
const CANVAS_BLEED_MINIMUM_WIRED_IDS = [
...
function main(): void {
  ...
  requireEqual(target.productWiring.wiredAxisCount, 58, "wired product axes");
  requireEqual(target.productWiring.remainingAxisCount, 7, "remaining product axes");
  ...
  const expectedWired = [...predecessorWired, ...CANVAS_BLEED_MINIMUM_WIRED_IDS].sort();
  const targetWired = target.productWiring.wiredLimitIds as string[];
  requireEqual(new Set(targetWired).size, 58, "unique wired product axes");
  ...
  requireEqual(target.productWiring.remainingGroups, [
    { group: "geometry", axisCount: 7, status: "PRODUCT_RESERVATION_POINTS_PENDING" },
  ...
  requireEqual(target.productWiring.geometryReservationProofAxisCount, 3,
  ...
  requireEqual(primary.boundary.wiredProductAxisCount, 58, "primary wired axes");
  requireEqual(primary.boundary.remainingProductAxisCount, 7, "primary remaining axes");
  ...
  const report = {
    ...
    targetManifest: {
      path: ".scratch/renderweave-template-v1/design-input-expression/capacity-component-target-v10.json",
    ...
    boundary: {
      ...
      wiredProductAxisCount: 58,
      remainingProductAxisCount: 7,
  ...
  process.stdout.write(
    `DESIGN_INPUT_EXPRESSION independent replay: ${observedCases.size}/195 PASS, 58/65 wired\n`,
  );
}
