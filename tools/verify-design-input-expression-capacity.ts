...
const TARGET_ID = "DESIGN_INPUT_EXPRESSION_TARGET::CAPACITY_AUTHORITY_PARTIAL_WIRING::16.0";
const IMPLEMENTATION_REVISION = "c235ca095f2c518cb9736a8158064c276a0f9d73";
...
const ROTATION_MINIMUM_WIRED_IDS = [
...
function main(): void {
  ...
  requireEqual(target.productWiring.wiredAxisCount, 64, "wired product axes");
  requireEqual(target.productWiring.remainingAxisCount, 1, "remaining product axes");
  ...
  requireEqual(predecessor.productWiring.geometryReservationProofAxisCount, 8,
    "predecessor Geometry reservation proof axis count");
  const expectedWired = [...predecessorWired, ...ROTATION_MINIMUM_WIRED_IDS].sort();
  const targetWired = target.productWiring.wiredLimitIds as string[];
  requireEqual(new Set(targetWired).size, 64, "unique wired product axes");
  ...
  requireEqual(target.productWiring.remainingGroups, [
    { group: "geometry", axisCount: 1, status: "PRODUCT_RESERVATION_POINTS_PENDING" },
  ...
  requireEqual(target.productWiring.geometryReservationProofAxisCount, 9,
  ...
  requireEqual(primary.boundary.wiredProductAxisCount, 64, "primary wired axes");
  requireEqual(primary.boundary.remainingProductAxisCount, 1, "primary remaining axes");
  ...
  const report = {
    ...
    targetManifest: {
      path: ".scratch/renderweave-template-v1/design-input-expression/capacity-component-target-v16.json",
    ...
    boundary: {
      ...
      wiredProductAxisCount: 64,
      remainingProductAxisCount: 1,
  ...
  process.stdout.write(
    `DESIGN_INPUT_EXPRESSION independent replay: ${observedCases.size}/195 PASS, 64/65 wired\n`,
  );
}
