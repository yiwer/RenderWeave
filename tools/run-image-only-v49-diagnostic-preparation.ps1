...
if (-not $resolvedEvidenceDir.StartsWith(
    ...
    throw 'IMAGE_ONLY v49 diagnostic preparation evidence must be below .sdlc/evidence.'
}
if (-not (Test-Path -LiteralPath $resolvedInputDirectory -PathType Container) -or
    ...
    throw 'IMAGE_ONLY v49 diagnostic preparation input directory is invalid.'
}
...
if (Test-Path -LiteralPath $summaryPath) {
    throw 'IMAGE_ONLY v49 diagnostic preparation summary already exists.'
}

Push-Location $repoRoot
try {
    & python.exe tools\test_verify_image_only_v49_diagnostic_preparation.py
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v49 diagnostic verifier tests failed with exit code $LASTEXITCODE."
    }
    & python.exe tools\verify_image_only_v49_diagnostic_preparation.py `
        --repository $repoRoot `
        --input-directory $resolvedInputDirectory `
        --output $summaryPath
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $summaryPath -PathType Leaf)) {
        throw 'IMAGE_ONLY v49 diagnostic verifier did not produce a passing summary.'
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v49 diagnostic inference tests failed with exit code $LASTEXITCODE."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v49 diagnostic PostgreSQL tests failed with exit code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
...
foreach ($forbidden in @(
    ...
    if ($summaryRaw.IndexOf(
        ...
        throw 'IMAGE_ONLY v49 diagnostic preparation summary is not payload-free.'
    }
}
...
if ($summary.result -cne 'PASS' -or
        ...
        $summary.normalizationIdentity -cne
            'renderweave-image-only-fresh-normalization/1.0:3096deba42aeab03be175074e6717ccf6898d4a628950d19eaa6891674d62375' -or
    ...
    throw 'IMAGE_ONLY v49 diagnostic Provider-zero summary contract failed.'
}

Write-Host 'IMAGE_ONLY v49 diagnostic preparation gate: PASS'
