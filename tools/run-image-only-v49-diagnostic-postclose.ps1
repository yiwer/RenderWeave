...
if (-not $resolvedEvidenceDir.StartsWith(
    ...
    throw 'IMAGE_ONLY v49 diagnostic post-close evidence must be below .sdlc/evidence.'
}
if (-not (Test-Path -LiteralPath $resolvedInputDirectory -PathType Container) -or
    ...
    throw 'IMAGE_ONLY v49 diagnostic post-close input directory is invalid.'
}
...
$summaryPath = Join-Path $resolvedEvidenceDir 'image-only-v49-diagnostic-postclose-summary.json'
if (Test-Path -LiteralPath $summaryPath) {
    throw 'IMAGE_ONLY v49 diagnostic post-close summary already exists.'
}

Push-Location $repoRoot
try {
    & python.exe tools\test_verify_image_only_v49_diagnostic_postclose.py
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v49 post-close verifier tests failed with exit code $LASTEXITCODE."
    }
    & python.exe tools\verify_image_only_v49_diagnostic_postclose.py `
        --repository $repoRoot `
        --input-directory $resolvedInputDirectory `
        --output $summaryPath
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $summaryPath -PathType Leaf)) {
        throw 'IMAGE_ONLY v49 post-close verifier did not produce a passing summary.'
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v49 post-close inference tests failed with exit code $LASTEXITCODE."
    }

    $appTests = @(
        'PostgresCertificationStageExecutionStoreTest#v49ScriptedProviderClosesReviewAndNegativeDiagnosticPathsWithoutReservations',
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v49 post-close PostgreSQL tests failed with exit code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
...
foreach ($forbidden in @(
    ...
    if ($summaryRaw.IndexOf(
        ...
        throw 'IMAGE_ONLY v49 diagnostic post-close summary is not payload-free.'
    }
}
$summary = $summaryRaw | ConvertFrom-Json
if ($summary.result -cne 'PASS' -or
        $summary.lifecycle -cne 'TERMINAL_CLOSED' -or
        $summary.terminalResult -cne 'FAILED' -or
        $summary.authorizationStatus -cne 'CLOSED' -or
        $summary.terminalReason -cne 'VISUAL_GROUNDING_REGION_FIELDS_INVALID' -or
        [int]$summary.diagnosticProviderUsage.startedRuns -ne 1 -or
        [int]$summary.diagnosticProviderUsage.providerCalls -ne 5 -or
        [int64]$summary.diagnosticProviderUsage.modelTokens -ne 67373 -or
        [int64]$summary.diagnosticProviderUsage.costMicrosCny -ne 1086900 -or
        [int]$summary.diagnosticProviderUsage.unsettledReservations -ne 0 -or
        [int64]$summary.goalModelTokenUsage -ne 67373 -or
        [int64]$summary.goalModelTokensRemaining -ne 1432627 -or
        [int]$summary.openAuthorizationCount -ne 0 -or
        [int]$summary.postcloseVerificationProviderUsage.attempts -ne 0 -or
        [int]$summary.postcloseVerificationProviderUsage.reservations -ne 0 -or
        [int64]$summary.postcloseVerificationProviderUsage.modelTokens -ne 0 -or
        [int64]$summary.postcloseVerificationProviderUsage.costMicrosCny -ne 0 -or
        [int]$summary.postcloseVerificationProviderUsage.apiKeyReads -ne 0 -or
        [bool]$summary.automaticRerunAllowed -or
        [bool]$summary.reviewPackCreated -or
    ...
    throw 'IMAGE_ONLY v49 diagnostic post-close summary contract failed.'
}

Write-Host 'IMAGE_ONLY v49 diagnostic post-close gate: PASS'
