...
try {
    ...
    if ($registryPrimary.status -ne 'PASS' -or $registryPrimary.checkCount -ne 22838 `
            -or $registryPrimary.failureCount -ne 0 `
            -or $registryIndependent.status -ne 'PASS' `
            -or $registryIndependent.checkCount -ne 22746 `
            -or $registryIndependent.failureCount -ne 0) {
        throw 'Template SPEC_REGISTRY replay counts or status drifted.'
    }
    if ($targetManifest.implementationRevision -ne 'spec-registry-bootstrap/1.13' `
            -or $targetManifest.artifacts.Count -ne 382 -or $ticket19.Count -ne 1 `
    ...
    if ($registryA2.status -ne 'PASS' `
            ...
            -or $registryA2.observedFrontier.capacityRecordsIssued -ne 0 `
