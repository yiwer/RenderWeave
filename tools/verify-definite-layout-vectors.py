...
def apply_grid_auto_constraints(
    children: list[Any],
    axis: str,
    auto_indices: list[int],
    sizes: list[float],
    ...
    constraints: list[tuple[int, int, int, tuple[int, ...], float]] = []
    for materialized_order, raw_child in enumerate(children):
        ...
        constraints.append(
            (
                ...
                start,
                materialized_order,
                tuple(covered_auto_indices),
                contribution if contribution > 0.0 else 0.0,
            )
        )

    constraints.sort(key=lambda constraint: constraint[:3])
    for span, start, _materialized_order, covered_auto, contribution in constraints:
        ...
        if len(covered_auto) == 1:
            auto_index = covered_auto[0]
            updated = sizes[auto_index] + deficit
            if not math.isfinite(updated):
                raise Unsupported("GRID_AUTO_TRACK", grid_occurrence)
            sizes[auto_index] = updated if updated > 0.0 else 0.0
            continue

        equal_increase = deficit / float(len(covered_auto))
        allocated = 0.0
        pending_updates: list[tuple[int, float]] = []
        for position, auto_index in enumerate(covered_auto):
            is_last = position + 1 == len(covered_auto)
            increase = deficit - allocated if is_last else equal_increase
            updated = sizes[auto_index] + increase
            if (
                not math.isfinite(increase)
                or increase < 0.0
                or not math.isfinite(updated)
            ):
                raise Unsupported("GRID_AUTO_TRACK", grid_occurrence)
            pending_updates.append((auto_index, updated if updated > 0.0 else 0.0))
            if not is_last:
                allocated += increase
                if not math.isfinite(allocated):
                    raise Unsupported("GRID_AUTO_TRACK", grid_occurrence)
        for auto_index, updated in pending_updates:
            sizes[auto_index] = updated
...
def measure_and_allocate_stack_children(
    ...
    if len(fill_indices) > 1:
        ...
        try:
            allocations = stack_main_fill_allocations(
                children,
                fill_indices,
                direction,
                available,
...
def stack_main_fill_allocations(
    children: list[Any],
    fill_indices: list[int],
    direction: str,
    available: float,
    ...
    if len(active_bounds) == 2 and len(allocations) == 2:
        ...
        for position, active_kind, frozen_bound in active_bounds:
            ...
            if (
                ...
                or maximum < minimum
                or frozen_bound != minimum
            ):
                raise Unsupported("STACK_MAIN_FILL", first_occurrence)
            minimum_sum += minimum
            if not math.isfinite(minimum_sum):
                raise Unsupported("STACK_MAIN_FILL", first_occurrence)
            allocations[position] = (
                allocations[position][0],
                minimum if minimum > 0.0 else 0.0,
            )
...
def verify(
    ...
    verifier.require(
        vectors["vectorVersion"] == "renderweave-definite-layout-vectors/57",
    ...
    expected_boundary = {
        ...
        "layoutImplementation": "RESOURCE_FREE_DEFINITE_ABSOLUTE_STACK_SINGLE_AND_INACTIVE_BOUND_OR_EXACT_TWO_FILL_SINGLE_ACTIVE_BOUND_WITHIN_REMAINING_OR_SINGLE_ACTIVE_MIN_OVERFLOW_OR_EXACT_TWO_FILL_TWO_MIN_SECOND_FREEZE_OVERFLOW_OR_EXACT_TWO_FILL_MIXED_ACTIVE_MIN_SECOND_MIN_FREEZE_OVERFLOW_OR_EXACT_TWO_FILL_MIXED_ACTIVE_MIN_SECOND_MIXED_MIN_FREEZE_OVERFLOW_OR_EXACT_TWO_FILL_MIXED_ACTIVE_MIN_SECOND_MIXED_TERMINAL_INACTIVE_OR_EXACT_TWO_FILL_SIMULTANEOUS_MIXED_MIN_FREEZE_OVERFLOW_OR_EXACT_TWO_FILL_TWO_MAX_SECOND_FREEZE_FREE_JUSTIFY_OR_EXACT_TWO_FILL_MIXED_ACTIVE_MAX_SECOND_MAX_FREEZE_FREE_JUSTIFY_OR_EXACT_THREE_FILL_SINGLE_ACTIVE_BOUND_ONE_REDISTRIBUTION_OR_EXACT_THREE_FILL_POST_REDISTRIBUTION_INACTIVE_BOUNDS_OR_EXACT_THREE_FILL_SECOND_MIN_FREEZE_LAST_REMAINDER_OR_EXACT_THREE_FILL_SECOND_MAX_FREEZE_LAST_REMAINDER_OR_EXACT_THREE_FILL_SECOND_MAX_FREEZE_TERMINAL_INACTIVE_MIN_OR_EXACT_THREE_FILL_SECOND_MAX_FREEZE_TERMINAL_INACTIVE_MAX_OR_EXACT_THREE_FILL_THIRD_MAX_FREEZE_FREE_JUSTIFY_OR_EXACT_THREE_FILL_SINGLE_ACTIVE_MIN_OVERFLOW_OR_EXACT_THREE_FILL_SECOND_MIN_FREEZE_OVERFLOW_OR_EXACT_THREE_FILL_MIXED_ACTIVE_MIN_OVERFLOW_OR_EXACT_THREE_FILL_MIXED_ACTIVE_MIN_OVERFLOW_INACTIVE_UNFROZEN_MAX_OR_EXACT_THREE_FILL_MIXED_ACTIVE_MIN_OVERFLOW_SECOND_MIN_FREEZE_OVERFLOW_OR_EXACT_THREE_FILL_MIXED_ACTIVE_MIN_OVERFLOW_SECOND_MIXED_MIN_FREEZE_OVERFLOW_OR_EXACT_THREE_FILL_MIXED_ACTIVE_MIN_OVERFLOW_SECOND_MIXED_MIN_FREEZE_OVERFLOW_TERMINAL_INACTIVE_MAX_OR_EXACT_THREE_FILL_MIXED_ACTIVE_MIN_OVERFLOW_TWO_MIXED_MIN_FREEZES_OVERFLOW_OR_EXACT_THREE_FILL_MIXED_ACTIVE_MIN_OVERFLOW_MIXED_AND_MIN_ONLY_FREEZES_OVERFLOW_OR_EXACT_THREE_FILL_MIXED_ACTIVE_MIN_OVERFLOW_TWO_MIN_ONLY_FREEZES_OVERFLOW_MULTI_MAIN_FILL_AND_FIXED_SINGLE_FRACTION_INDEPENDENT_MULTI_AUTO_GRID_MULTI_AUTO_SPAN_STABLE_DEFICIT_GRID_DEFINITE_MULTI_FRACTION_LAST_REMAINDER_GRID_EMPTY_CONTAINER_STACK_HUG_GRID_AUTO_HUG_CONTRIBUTION_GRID_HUG_EXACT_QUARTER_TURN_AFFINE_FRAME_GROUP_HUG_FIXED_OPPOSITE_AXIS_CROSS_FILL_DEFINITE_ABSOLUTE_PARENT_OFFER_DEFINITE_STACK_CROSS_OUTER_OFFER_STACK_MAIN_FILL_CROSS_HUG_REMEASURE_NESTED_STACK_MAIN_OFFER_PROPAGATION_COLUMNS_FIRST_GRID_CELL_OUTER_OFFER_STACK_MAIN_OFFER_COLUMNS_FIRST_GRID_CROSS_HUG_ABSOLUTE_PARENT_OFFER_COLUMNS_FIRST_GRID_CROSS_HUG_GRID_CELL_OFFER_COLUMNS_FIRST_NESTED_GRID_CROSS_HUG_GRID_CELL_OFFER_STACK_MAIN_FIRST_CROSS_HUG_DIRECTION_CHANGING_STACK_CROSS_OFFER_MAIN_HUG_NESTED_STACK_RESOLVED_OPPOSITE_OFFER_RECURSION_COLUMNS_FIRST_GRID_TERMINAL_NORMALIZATION_BOX_KERNEL",
    ...
    verifier.require(len(vectors["laidOutCases"]) == 261, "laid-out case count drifted")
    verifier.require(
    ...
    return {
        "verifier": "renderweave-definite-layout-python-independent/57",
        "result": "PASS",
        "assurance": "A2",
