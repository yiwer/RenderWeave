...
if (-not $resolvedEvidenceDir.StartsWith(
    ...
    throw 'IMAGE_ONLY v52 post-close evidence must be below .sdlc/evidence.'
}
if (-not (Test-Path -LiteralPath $resolvedInputDirectory -PathType Container) -or
    ...
    throw 'IMAGE_ONLY v52 post-close input directory is invalid.'
}
...
$summaryPath = Join-Path $resolvedEvidenceDir 'image-only-v52-diagnostic-postclose-summary.json'
if (Test-Path -LiteralPath $summaryPath) {
    throw 'IMAGE_ONLY v52 post-close summary already exists.'
}

Push-Location $repoRoot
try {
    & python.exe tools\test_verify_image_only_v52_diagnostic_postclose.py
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v52 post-close verifier tests failed with exit code $LASTEXITCODE."
    }
    & python.exe tools\verify_image_only_v52_diagnostic_postclose.py `
    ...
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $summaryPath -PathType Leaf)) {
        throw 'IMAGE_ONLY v52 post-close verifier did not produce a passing summary.'
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v52 post-close inference tests failed with exit code $LASTEXITCODE."
    }

    $env:RENDERWEAVE_RUN_V52_PROFILE_SUCCESSOR_DIAGNOSTIC = 'true'
    $appTests = @(
        'ImageOnlyV52ProfileSuccessorDiagnosticLiveTest',
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v52 post-close app tests failed with exit code $LASTEXITCODE."
    }
}
finally {
    $env:RENDERWEAVE_RUN_V52_PROFILE_SUCCESSOR_DIAGNOSTIC = $null
    Pop-Location
}
...
foreach ($forbidden in @(
    ...
    if ($summaryRaw.IndexOf($forbidden, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
        throw 'IMAGE_ONLY v52 post-close summary is not payload-free.'
    }
}
$summary = $summaryRaw | ConvertFrom-Json
if ($summary.result -cne 'PASS' -or
        $summary.authorizationStatus -cne 'CLOSED' -or
        $summary.diagnosticResult -cne 'REVIEW_REQUIRED' -or
        $summary.manualReviewStatus -cne 'PENDING_OWNER_REVIEW' -or
        [int]$summary.itemParentEnvelopeNormalized -ne 1 -or
        [int]$summary.openAuthorizationCount -ne 0 -or
        [int]$summary.providerCalls -ne 3 -or
        [int64]$summary.modelTokens -ne 37451 -or
        [int64]$summary.costMicrosCny -ne 521364 -or
        [int]$summary.unsettledReservations -ne 0 -or
        [int64]$summary.goalAggregateModelTokens -ne 159069 -or
        [int64]$summary.goalRemainingModelTokens -ne 1340931 -or
        [int]$summary.postcloseProviderUsage.attempts -ne 0 -or
        [int]$summary.postcloseProviderUsage.apiKeyReads -ne 0 -or
    ...
    throw 'IMAGE_ONLY v52 post-close summary contract failed.'
}

Write-Host 'IMAGE_ONLY v52 diagnostic post-close gate: PASS'
