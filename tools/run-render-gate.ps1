...
try {
    ...
    $definiteLayoutIndependent = Get-Content -Raw -Encoding UTF8 -LiteralPath $definiteLayoutReport |
        ConvertFrom-Json
    if ($definiteLayoutIndependent.verifier -ne 'renderweave-definite-layout-python-independent/15' `
            -or $definiteLayoutIndependent.result -ne 'PASS' `
            -or $definiteLayoutIndependent.assurance -ne 'A2' `
            -or $definiteLayoutIndependent.laidOutCases -ne 74 `
            -or $definiteLayoutIndependent.unsupportedCases -ne 14 `
            -or $definiteLayoutIndependent.passed -ne 88 `
            -or $definiteLayoutIndependent.total -ne 88 `
            -or $definiteLayoutIndependent.failed -ne 0 `
            -or $definiteLayoutIndependent.checks -ne 264 `
            ...
            -or $definiteLayoutIndependent.layoutImplementation -ne 'RESOURCE_FREE_DEFINITE_ABSOLUTE_STACK_SINGLE_MAIN_FILL_AND_FIXED_SINGLE_FRACTION_INDEPENDENT_MULTI_AUTO_GRID_EMPTY_CONTAINER_STACK_HUG_GRID_AUTO_HUG_CONTRIBUTION_GRID_HUG_EXACT_QUARTER_TURN_AFFINE_FRAME_GROUP_HUG_FIXED_OPPOSITE_AXIS_CROSS_FILL_NORMALIZATION_BOX_KERNEL' `
    ...
    $summary = [ordered]@{
        ...
        boundary = [ordered]@{
            ...
            layoutKernel = 'RESOURCE_FREE_DEFINITE_ABSOLUTE_STACK_SINGLE_MAIN_FILL_AND_FIXED_SINGLE_FRACTION_INDEPENDENT_MULTI_AUTO_GRID_EMPTY_CONTAINER_STACK_HUG_GRID_AUTO_HUG_CONTRIBUTION_GRID_HUG_EXACT_QUARTER_TURN_AFFINE_FRAME_GROUP_HUG_FIXED_OPPOSITE_AXIS_CROSS_FILL_NORMALIZATION_BOX_AUTOMATED_VERIFIED_UNWIRED'
