...
if (-not $resolvedEvidenceDir.StartsWith(
    ...
    throw 'IMAGE_ONLY v48 successor evidence directory must be below .sdlc/evidence.'
}
...
if (-not (Test-Path -LiteralPath $resolvedInputDirectory -PathType Container) -or
    ...
    throw 'IMAGE_ONLY v48 successor input directory is invalid.'
}
...
$summaryPath = Join-Path $resolvedEvidenceDir 'image-only-v48-successor-summary.json'
if (Test-Path -LiteralPath $summaryPath) {
    throw 'IMAGE_ONLY v48 successor summary already exists.'
}

Push-Location $repoRoot
try {
    & python.exe tools\test_verify_image_only_v48_successor.py
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v48 independent verifier tests failed with exit code $LASTEXITCODE."
    }
    & python.exe tools\verify_image_only_v48_successor.py `
        --repository $repoRoot `
        --input-directory $resolvedInputDirectory `
        --output $summaryPath
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $summaryPath -PathType Leaf)) {
        throw 'IMAGE_ONLY v48 independent verification did not produce a passing summary.'
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v48 inference tests failed with exit code $LASTEXITCODE."
    }
    ...
    $appTests = 'PostgresCertificationStageExecutionStoreTest,' +
        ...
        'pipelineFourPointThirtyFailsAnUnlistedObserveCodeWithoutAnotherReservation+' +
        'pipelineFourPointThirtyFeedsFieldSpecificEvidenceCodeIntoOneBoundedCorrection+' +
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v48 PostgreSQL tests failed with exit code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
...
foreach ($forbidden in @(
    ...
    if ($summaryRaw.IndexOf(
        ...
        throw 'IMAGE_ONLY v48 successor summary is not payload-free.'
    }
}
...
if ($summary.result -cne 'PASS' -or
        ...
        [int]$summary.diagnosticProviderUsage.providerCalls -ne 1 -or
        [int64]$summary.diagnosticProviderUsage.modelTokens -ne 13394 -or
        [int64]$summary.diagnosticProviderUsage.costMicrosCny -ne 218928 -or
        ...
        [int]$summary.verificationProviderUsage.apiKeyReads -ne 0) {
    throw 'IMAGE_ONLY v48 successor Provider-zero summary contract failed.'
}

Write-Host 'IMAGE_ONLY v48 successor Provider-zero gate: PASS'
