...
MODULE_PATH = Path(__file__).with_name("verify_image_only_v51_diagnostic_preparation.py")
SPEC = importlib.util.spec_from_file_location("v51_diagnostic", MODULE_PATH)
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(MODULE)


class V51DiagnosticPreparationVerifierTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_replays_exact_v51_bound_identities_and_caps(self):
    ...
    def test_identity_classifier_and_cap_tampering_fail_closed(self):
        for field, value, expected in (
            ("successorImplementationIdentity",
             "renderweave-image-only-v51-implementation/1.0:" + "f" * 64,
             "V51_DIAGNOSTIC_PREPARATION_FACT_DRIFT"),
            ("parentContainmentPrimaryCode", "VISUAL_GROUNDING_PARENT_CONTAINMENT_INVALID",
             "V51_DIAGNOSTIC_PREPARATION_FACT_DRIFT"),
            ("maximumProviderCalls", 6, "V51_DIAGNOSTIC_J1_CAPS_DRIFT"),
        ):
    ...
    def test_any_open_authorization_fails_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                "V51_DIAGNOSTIC_OPEN_AUTHORIZATION_FORBIDDEN", str(failure.exception)
            )
