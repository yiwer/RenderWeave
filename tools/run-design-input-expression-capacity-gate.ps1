...
try {
    ...
    if ($result.reportVersion -ne `
            ...
            -or $result.boundary.wiredProductAxisCount -ne 62 `
            -or $result.boundary.remainingProductAxisCount -ne 3 `
    ...
    Write-Host 'DESIGN_INPUT_EXPRESSION component gate: 195/195 scalar PASS, 62/65 wired'
}
finally {
    Pop-Location
}
