...
try {
    ...
    if ($result.reportVersion -ne `
            ...
            -or $result.boundary.wiredProductAxisCount -ne 61 `
            -or $result.boundary.remainingProductAxisCount -ne 4 `
    ...
    Write-Host 'DESIGN_INPUT_EXPRESSION component gate: 195/195 scalar PASS, 61/65 wired'
}
finally {
    Pop-Location
}
