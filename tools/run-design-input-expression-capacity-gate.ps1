...
$target = Join-Path $repoRoot `
    '.scratch\renderweave-template-v1\design-input-expression\capacity-component-target-v5.json'
...
try {
    & mvn.cmd -B -ntp -pl renderweave-app -am `
        '-Dtest=cn.hbads.renderweave.template.internal.DesignInputExpressionCapacityConformanceTest,cn.hbads.renderweave.template.internal.DesignDslCapacityReservationTest,cn.hbads.renderweave.template.internal.DesignDslSemanticCapacityReservationTest,cn.hbads.renderweave.template.internal.ExpressionDefinitionCapacityReservationTest,cn.hbads.renderweave.template.internal.TemplateProblemCapacityReservationTest,cn.hbads.renderweave.rendering.internal.RenderInputCapacityReservationTest,cn.hbads.renderweave.app.rendering.RenderingControllerCapacityReservationTest,cn.hbads.renderweave.app.rendering.RenderingApplicationConfigurationTest,cn.hbads.renderweave.app.template.TemplateApplicationConfigurationTest' `
    ...
    if ($result.reportVersion -ne `
            ...
            -or $result.boundary.wiredProductAxisCount -ne 46 `
            -or $result.boundary.remainingProductAxisCount -ne 19 `
    ...
    Write-Host 'DESIGN_INPUT_EXPRESSION component gate: 195/195 scalar PASS, 46/65 wired'
}
finally {
    Pop-Location
}
