...
MODULE_PATH = Path(__file__).with_name("verify_image_only_v49_diagnostic_preparation.py")
SPEC = importlib.util.spec_from_file_location("v49_diagnostic", MODULE_PATH)
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(MODULE)


class V49DiagnosticPreparationVerifierTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_replays_exact_fresh_identities_and_caps(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                MODULE.NORMALIZATION_IDENTITY, identities["normalizationIdentity"]
            )

    def test_normalization_and_cap_tampering_fail_closed(self):
        for field, value, expected in (
            ("normalizationIdentity", MODULE.NORMALIZATION_VERSION + ":" + "f" * 64,
             "V49_DIAGNOSTIC_PREPARATION_FACT_DRIFT"),
            ("maximumProviderCalls", 6, "V49_DIAGNOSTIC_J1_CAPS_DRIFT"),
        ):
            with self.subTest(field=field), tempfile.TemporaryDirectory() as directory:
                root = Path(directory)
                path = self.copy_preparation(root)
                document = json.loads(path.read_text(encoding="utf-8"))
                if field == "normalizationIdentity":
    ...
    def test_any_open_authorization_fails_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                "V49_DIAGNOSTIC_OPEN_AUTHORIZATION_FORBIDDEN", str(failure.exception)
            )
