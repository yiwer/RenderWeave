...
if (-not $resolvedEvidenceDir.StartsWith(
    ...
    throw 'IMAGE_ONLY v51 diagnostic preparation evidence must be below .sdlc/evidence.'
}
if (-not (Test-Path -LiteralPath $resolvedInputDirectory -PathType Container) -or
    ...
    throw 'IMAGE_ONLY v51 diagnostic preparation input directory is invalid.'
}
...
if (Test-Path -LiteralPath $summaryPath) {
    throw 'IMAGE_ONLY v51 diagnostic preparation summary already exists.'
}

Push-Location $repoRoot
try {
    & python.exe tools\test_verify_image_only_v51_diagnostic_preparation.py
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v51 diagnostic verifier tests failed with exit code $LASTEXITCODE."
    }
    & python.exe tools\verify_image_only_v51_diagnostic_preparation.py `
        --repository $repoRoot `
        --input-directory $resolvedInputDirectory `
        --output $summaryPath
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $summaryPath -PathType Leaf)) {
        throw 'IMAGE_ONLY v51 diagnostic verifier did not produce a passing summary.'
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v51 diagnostic inference tests failed with exit code $LASTEXITCODE."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "IMAGE_ONLY v51 diagnostic PostgreSQL tests failed with exit code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
...
foreach ($forbidden in @(
    ...
    if ($summaryRaw.IndexOf(
        ...
        throw 'IMAGE_ONLY v51 diagnostic preparation summary is not payload-free.'
    }
}
...
if ($summary.result -cne 'PASS' -or
        ...
        $summary.normalizationIdentity -cne
            'renderweave-image-only-fresh-normalization/1.0:632c601ccdcbd561fcb9502777a888712a564a81352f9f19b163b4a0e9a6b4cc' -or
        $summary.parentContainmentPrimaryCode -cne
            'VISUAL_GROUNDING_PARENT_CONTAINMENT_CLASSIFIED' -or
        $summary.successorImplementationIdentity -cne
            'renderweave-image-only-v51-implementation/1.0:45f3e9d6b25a0fbe9788fd7714adecaabc800a9279c2e37d73e76f850df887f1' -or
    ...
    throw 'IMAGE_ONLY v51 diagnostic Provider-zero summary contract failed.'
}

Write-Host 'IMAGE_ONLY v51 diagnostic preparation gate: PASS'
