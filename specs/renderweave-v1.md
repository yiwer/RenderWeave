...
```text
NORMALIZE
→ MULTISCALE_VIEW / DOCUMENT_VISION (IMAGE_ONLY product-v42: local, exact capability)
...
grounding 为结构事实源，但当前新建产品 live 目录只接纳 IMAGE_ONLY product-v42。历史 run 继续按其不可变
snapshot 恢复。`HIERARCHY` 与 `ELEMENT_BINDING` 用于 IMAGE_ONLY product-v42。
...
- product-v42 使用启动时探测、默认零网络的 Document Vision 窄端口；其 capability 必须与 Profile 精确绑定，
  缺失或不匹配时在创建/retry 和 Provider reservation 前 fail-closed。
- product-v42 继承 product-v41 的早期 GROUP prerequisite：在已严格解码的 HIERARCHY response 含 relationship、而 OBSERVE inventory 为 0 GROUP 时，
...
- IMAGE_ONLY product-v42 对独立可寻址/有序的重复项建立 child CandidateSchema；即使主值是文本，已有
  顺序、当前状态、换乘标记等 item-level 属性或明确重复组时也不得降级为 `Array<TEXT>`。
- IMAGE_ONLY product-v42 的最终 Candidate 必须完整保持已验证计划中的根、实体、关系基数、字段归属和
...
- IMAGE_ONLY 若同一 artifact 的至少两个 bbox 全部位于像素边界、并集横纵覆盖至少 50% 画布且到达右侧或
  底部 98% 边界，可在 validator 前按 artifact 尺寸确定性换算为 0..10000，并保留 WARNING；product-v42
...
- 新建产品 live run 只提供 `dashscope-qwen37-plus-product-v42-hybrid-generic`（`qwen3.7-plus`）、
  `dashscope-qwen38-max-product-v42-hybrid-generic`（`qwen3.8-max`）和
  `dashscope-qwen37-flash-product-v42-hybrid-generic`（`qwen3.7-flash`）三份
  IMAGE_ONLY Profile；Web 默认 Plus，Max 用于用户显式选择的高难嵌套任务，Flash 只定位为低成本 smoke。
  历史 canary/certification、全部 product-v41 及更早 Profile 保持不可变、可读和可恢复，但不进入
  新建产品选择器。v42 逐字继承 v41 的 pipeline/prompt/Document Vision/价格与严格验证，只把运行上限固定为
  七次调用和单步 360 秒；Plus/Flash 使用官方 capability 支持的 16384 output tokens，缺少 advertised output
  上限的 exact `qwen3.8-max` 继续保持 8192。v42 不继承 v41 的 live 或质量证据。三份当前 v42 Profile 均为
  `EXPERIMENTAL`，也不做跨模型自动升级路由。
...
- product-v42 的三个 Profile 将 `maximumEstimatedCostMicrosCny` 统一固定为 2,000,000（¥2）。这是
  每次调用的保守预留上界而非实际收费；若高分辨率/多图请求的保守估值超过 ¥2，必须在零 Provider
  调用处拒绝，不能靠任务累计限额绕过。
- product-v42 的 Plus/Flash 单步输出上限为 16384 tokens，Max 因能力证据不足保持 8192；输出合同均不超过
  262144 bytes。这只为有证据支持的模型提供表达空间，不放宽 ¥2 单次预留门或七次总调用上限。
- product-v42 的每个 Provider stage 请求时限为 360 秒；
...
- product-v42 产品 run 必须设置 `costLimitMicrosCny`，范围 1..5,000,000（最多 ¥5）；全部串行阶段、阶段重试
...
推断入口和审核详情使用一致的四步进度：准备输入 → 受控识别 → 逐项校对 → 原子创建。运行中展示人类可读 stage；状态机许可时可取消，FAILED/CANCELLED 只允许显式 retry 创建新 run。上传选择必须提供逐文件检查和移除，但选择、预览或切换 Profile 均不得触发 Provider。产品模型选择器只展示三份 IMAGE_ONLY product-v42 Profile，默认 Plus，并对每份 Profile 显示独立 readiness；capability 未就绪的选项不可启动。当前部署要求每个新任务设置不超过 ¥5 的累计成本硬上限，按钮是否可用不再依赖历史 canary 账本剩余额度。
