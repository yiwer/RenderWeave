...
- 2026-08-18 **IOPA-P2-06 No-IP UDS OCR sidecar 实现 slice 已 `automated_verified`，停在稳定节点**：
  `docker/ocr-sidecar/` 交付 stdlib HTTP/1.1-over-UDS server（`/health`/`/capability`/`/ocr`，envelope 与
  stdio 适配器同形）、启动阻塞 capability+固定输出 synthetic probe、构建期从 vendored RapidOCR wheel 提取
  三份 ONNX 并 SHA-256 校验、non-root UID 10001/read-only 模型/BLAS 单线程确定性环境。base image
  `python:3.12-slim-bookworm@sha256:356b0d18…` digest pin，20 个 vendored wheel 全量 `--require-hashes`
  离线安装（opencv-python-headless 覆盖元数据免 mesa 闭包、内部 antlr 4.9.3 wheel），`--network=none`
  强制零下载。Java 新增 `UnixDomainSocketDocumentVisionRunner`（AF_UNIX+看门狗）经 `forUnixSocket` 复用
  ProcessRunner seam；compose `ocr-sidecar` 服务 no-IP/read-only/cap-drop/2CPU/2GiB/PID64。容器内 R0
  equivalence：stdio 与 UDS 同请求输出 **byte-identical**（4 lines）。capability identity 不变。
  **人工阻断未满足：license J1=`J0_PENDING`，capability 未准入**；SBOM/CVE/malware/attestation 归 release
  gate 链。ADR-0053 accepted。A1 gate `.sdlc/evidence/20260818-235714-image-only-p2-ocr-sidecar/` PASS：
  verifier 2/2 + 探针全绿 + app 回归 18/18；summary SHA-256=
  ...
  image=`sha256:76d836f38b392e762d927ff899badcce2a49d71a3cecfcc6558772dc4528f9a8`。
  Provider/key usage=0、OPEN=0、未 grant/apply/publish/deploy/commit/push。
  下一 frontier=IOPA-P3-01 Production admission 与 9-value readiness。
- 2026-08-18 **IOPA-P2-05 payload-free audit chain 与双开关已 `automated_verified`，停在稳定节点**：
  V026 建立逐 run 单调 sequence + domain-separated digest chain 的 Live Admission Audit；event 只含 opaque
  identity/digest/固定 code/usage/cost/时间；append-only 触发器与 Flyway owner/`renderweave_live_runtime`
  role 分离（runtime 无 UPDATE/DELETE），independent replay 对 duplicate/reorder/delete/tamper/missing 均
  fail closed。`PostgresLiveProviderCallGate` 把 call authorization、attempt identity、费用 reservation 与
  audit event 放进同一 PostgreSQL 事务，提交后才产生 Provider permit；crash-after-commit 不盲重放。
  `ImageOnlyAdmissionPolicy`（append-only 版本化，默认关闭）与 `ProviderEgressPermit`（orchestrator-mounted
  只读 port，默认关闭）互不推导：00/01/10 全拒，仅 11 通过；QUEUED 排空为稳定终态、重开不复活、
  REVIEW_REQUIRED 不受阻；audit 不可写/链异常投影 `AUDIT_INTEGRITY_UNAVAILABLE`。payload canary
  （文件名/图片签名/OCR/response/PII/secret/CoT）在 audit/events/attempts/run 投影零泄漏；独立 Python
  verifier 重放 Java 导出链。ADR-0052 accepted。A1 gate
  `.sdlc/evidence/20260818-204141-image-only-p2-audit-dual-switch/` PASS：verifier 3/3、inference 15/15、
  app 受影响回归 149/149，总计 164/164；summary SHA-256=
  ...
  Provider/key usage=0、OPEN=0、未 grant/apply/publish/deploy/commit/push；P2-04 material 由本 gate 受影响
  回归重验接续。下一 frontier=IOPA-P2-06 No-IP UDS OCR sidecar。
- 2026-08-18 **新 session 接手 IMAGE_ONLY Production Admission（接手记录）**：已按
  `plans/image-only-production-admission-handoff-20260818-p2-04.md` 的接手顺序完整重锚定——AGENTS/CONSTITUTION/CONTEXT、
  approved delta `specs/changes/20260817-image-only-production-admission.md`、Blueprint v1、实施计划 §0.16/IOPA-P2-05/
  Gate/人工阻断/风险/Decision、源票 04 决策 8/13/14/29/31 与源票 11 的 audit/telemetry retention、低基数与
  Provider-zero drain 下游边界。接手时 `main` HEAD=`7848c821aa9b809dd8cadb2b5e28f40f6947a90e`（与 handoff 一致），
  dirty worktree 全量保留（275 项 tracked/untracked，不 reset/checkout/clean）。**生命周期确认：
  P2-01..04=automated_verified，P2-05=not-started（本 session 的唯一实施入口）；全部 live authorization 均 CLOSED**
  （spike JSON 顶层残留 `"status": "OPEN"` 为编辑遗留，其后 `CLOSED`+closedAt+closureReason 为有效终态，
  与 Blueprint 证据索引一致，不修改历史记录）。HEAD 只是规划基线，不把它当成完整实现；无生产配置、无
  commit/push/tag/deploy。P2-05=Provider-zero：payload-free Live Admission Audit chain + 双开关，
  Testcontainers PostgreSQL，不读取 Key、不 live、不 apply/publish。
- 新 Agent session 接续 IMAGE_ONLY Production Admission 时，先完整读取
  `plans/image-only-production-admission-handoff-20260819-p2-06.md`；它冻结 dirty-worktree 保护、P2-06 A1 evidence、
  权限边界与 P3-01 唯一下一入口。（P2-04/P2-05 handoff 保留为历史：
  `plans/image-only-production-admission-handoff-20260818-p2-04.md`、
  `plans/image-only-production-admission-handoff-20260818-p2-05.md`。）
- 2026-08-18 **IOPA-P2-04 payload expiry/tombstone/delete worker 已 `automated_verified`，并按所有者要求停在稳定节点**：
  V025 固化 first-upload 7-day immutable retention、shared-reference no-extension、24h retry/reupload boundary、
  tombstone-first read/retry/Provider/apply cutoff，以及 COMPLETED immediate / terminal≤24h / review day-7 expiry。
  payload-free deletion queue 以 lease/backoff/shared-reference/ingest-lease guard 异步清除 ciphertext+wrapped DEK；
  backlog>24h 投影 `PAYLOAD_DELETION_UNHEALTHY` 并关闭新 live admission。ADR-0051 accepted；scheduler 默认关闭。
  A1 `.sdlc/evidence/20260818-173253-image-only-p2-payload-lifecycle/` PASS：verifier 2/2、lifecycle PG 6/6、
  affected regression 117/117，总计 123/123；summary SHA-256=
  ...
  Provider/key usage=0、OPEN=0、未 grant/apply/publish/deploy/commit/push；下一 frontier=P2-05，但本轮未启动。
  Goal aggregate remains 159,069/1,500,000 tokens。
- 2026-08-18 **IOPA-P2-03 envelope-encrypted artifact store 已 `automated_verified`**：每个 persisted normalized
  artifact 使用 random 256-bit DEK + AES-256-GCM；Blob 仅存 ciphertext，V024 PostgreSQL 保存 immutable AEAD metadata
  与 wrapped DEK，orchestrator-mounted KEK ring 与 PG/Blob/backup/log/evidence 分域。artifact-scoped advisory transaction
  lock 支持 response-loss idempotency 与 encrypted crash-orphan reconciliation；DB/Blob/KEK 缺失及 tamper/truncate/swap
  均 fail closed。rotation 只 re-wrap DEK，payload ciphertext 不变，旧 KEK refcount=0 后才可销毁；delete 双清 wrapped
  DEK/ciphertext。ADR-0050 accepted。Provider-zero gate=
  `.sdlc/evidence/20260818-165229-image-only-p2-encryption/`，unit 5/5 + Testcontainers/filesystem integration 9/9 +
  verifier 2/2 PASS；summary SHA-256=`161cf4380b0cebce3c4c23eb05c1318a34d5d3a042540fcdc15e8cbb35f6c393`，
  implementation=`renderweave-image-only-p2-encryption/1.0:b1131f9b5130c331e4b041b3fcb01027f8b14c4b52c9f54f8341039b7ede950d`。
  encryption config 默认关闭，未配置/部署生产；Provider/key usage=0、OPEN=0、未 grant/apply/publish/commit/push。
  下一 frontier=IOPA-P2-04 expiry/tombstone/delete worker；Goal aggregate remains 159,069/1,500,000 tokens。
- 2026-08-18 **IOPA-P2-02 classification/notice/manifest/confirmation 已 `automated_verified`**：新增唯一
  `ImageOnlyProductionAdmission` create seam，只接纳 `USER_PROVIDED+ORDINARY_DESIGN` 与 1–10 static PNG/JPEG，
  hard caps=10MiB/25Mpx each、32MiB aggregate。server-owned immutable notice 绑定 policy/Provider contract/exact
  Profile/caps；ordered normalized `LiveInputManifest` 不含原文件名。V023 在同一 PostgreSQL transaction 内创建
  run+append-only notice/manifest/confirmation；fresh GatewayAssertion response-loss replay 返回原 confirmation，semantic
  drift typed conflict；15min first dispatch、2h calls-not-after、stale notice、manifest/Profile drift、ambiguous attempt
  均 fail closed。ADR-0049 accepted。Provider-zero gate=
  `.sdlc/evidence/20260818-163143-image-only-p2-confirmation/`，independent verifier 2/2、inference 23/23、
  Testcontainers PostgreSQL 4/4 PASS；summary SHA-256=
  ...
  Provider/key usage=0，未创建生产 confirmation/authority，未 apply/publish/deploy/commit/push；下一 frontier=P2-03
  encrypted artifact store。Goal aggregate remains 159,069/1,500,000 tokens。
- 2026-08-18 **IMAGE_ONLY v52 diagnostic closeout + IOPA-P2-01 automated_verified**：v52 one-shot
  diagnostic 到达 `REVIEW_REQUIRED`，3 calls / 37,451 model tokens / ¥0.521364；authorization、PostgreSQL
  ledger 与 harness 均 CLOSED，OPEN=0。isolated review pack SHA-256=
  `fe61bf267049a6fed74daad41d4ebeaca30012b19e2806a10be0e53e4f29ccdc`，仍等待所有者人工 verdict；没有
  certification credit、Candidate apply、StaticSchema publish 或生产部署。post-close A1=
  `.sdlc/evidence/20260818-154409-image-only-v52-diagnostic-postclose/`。
- P2-01 现由单一 GatewayAssertion authority 接受 Ed25519 compact JWS，绑定 opaque actor/request/jti、method/path
  与 mutation idempotency digest；60s TTL、30s assertion skew、UTC rollback fail-closed，V022 PostgreSQL 原子
  消费 mutation jti。gateway/API 与 Actuator 分别具备 exact mTLS leaf fingerprint seam，公共 nginx 对 Actuator
  固定 404 并清空客户端身份 header；ADR-0048 accepted。A1=
  `.sdlc/evidence/20260818-160325-image-only-p2-admission/`，6 inference + 8 app/PostgreSQL tests PASS，summary
  SHA-256=`65987841931e1ffc535fbc45ecb6c9a6166d79e2a37a4fa834a831b7a2973c13`；Provider attempts/reservations/
  tokens/cost/API-key reads 均 0。下一 Provider-zero frontier=IOPA-P2-02 classification/notice/manifest/confirmation；
  Goal aggregate remains 159,069/1,500,000 tokens。
- 2026-08-18 **IOPA-P1-R17..R18 v50 canonicalization successor 已 `automated_verified/resolved`**：exact hidden
  Profile=`dashscope-qwen38-max-product-v50-hybrid-generic`，SHA=`62f333aee7096f09d6d04dea004641e8b0a9c425ee133d09a563594d81200691`；
  pipeline 4.32，Prompt 16 SHA=`c01f11fb68d846b0fe6a71d42615dfb410d7d9945458f57b2f50748ef61451b5`。
  v49→v50 只改三 identity fields；v50 hidden/EXPERIMENTAL/ungranted/非 product-live。lossless canonicalizer 只映射
  r/e/g declaration/equality/reference graph，ambiguous/dangling/type/shape fail closed，完整 validator 仍执行；raw local IDs
  不进 evidence。A1 `.sdlc/evidence/20260818-124217-image-only-v50-successor/` PASS（verifier 2/2、inference
  79/79、Testcontainers PostgreSQL scripted Provider 3/3），v49 CLOSED artifacts 3/3 replay；OPEN=0、Provider usage=0。
  Goal live usage=67,373/1,500,000，下一步准备 fresh v50 non-scoring diagnostic；未 grant/apply/publish/deploy/
  commit/push。
- 2026-08-18 **IOPA-P1-R15 v49 diagnostic 已 `TERMINAL_CLOSED/FAILED`，R16 v50 source decision 已批准**：
  exact J1 `20260818-iopa-v49-diagnostic-432fdfeb` 的唯一 run `3f8a063b-…` 用满 5 calls，67,373 tokens、
  ¥1.086900、0 unsettled；attempts 0/3/4 的同一 mixed set 为 region ID + parent ID + repeat-group ID，第三次落账后
  无第 6 次 permit。0 Candidate/review pack、credit=0、next=false。live negative evidence
  `.sdlc/evidence/20260818-121611-image-only-v49-successor-diagnostic-live/`；post-close
  `.sdlc/evidence/20260818-122337-image-only-v49-diagnostic-postclose/` PASS（19/19 + PG 3/3），authorization/
  ledger CLOSED、OPEN=0。Goal usage=67,373/1,500,000，remaining=1,432,627。按 Blueprint Appendix A 与 standing
  approval，ticket 31/ADR-0044/approved v50 delta 已冻结只重命名 internal local-ID graph 的 deterministic
  canonicalizer；v50 尚未实现，禁止 patch/rerun v49，未 apply/publish/deploy/commit/push。
- 2026-08-18 **IOPA-P1-R14 fresh v49 diagnostic authority 已 `automated_verified/resolved`**：cycle=
  `432fdfeb-c5ab-4cff-92f4-e066a0d98c8c`；normalization=`/1.0:3096deba…d62375`，manifest=
  `/1.0:8ff24a61…f0c8e2`，evaluator=`/1.0:b2167261…55c29e`；exact v49 SHA 与单一获批 artifact、
  USER_PROVIDED + ORDINARY_DESIGN、non-scoring scope 已冻结。proposed authority=
  `20260818-iopa-v49-diagnostic-432fdfeb`，caps=1 run/5 calls/100k tokens/¥3/per-run 5+¥3/≤2h，当前
  `PENDING_J1`、OPEN=0。ADR-0043 显式绑定 normalization，保留 v47/v48 CLOSED bytes；one-shot harness 默认 skip，
  scripted Provider + PG 两条 closure path 全绿。A1 dedicated
  `.sdlc/evidence/20260818-115742-image-only-v49-diagnostic-preparation/` PASS（50/50 + PG 7/7），fast
  `.sdlc/evidence/20260818-115934-fast/`、server `.sdlc/evidence/20260818-120001-server/` PASS（20/13/389/287）。
  ticket 30 为当前 live frontier；Goal usage 0/1,500,000，未 apply/publish/deploy/commit/push。
- 2026-08-18 **IOPA-P1-R13 immutable v49 successor 已 `automated_verified/resolved`**：exact Profile=
  `dashscope-qwen38-max-product-v49-hybrid-generic`，canonical SHA=`acffdd4dd56ca2f1f7260fc5d37aa48ca3da488a0ae2718f2095bf1530e86eaf`；
  pipeline 4.31，Prompt 15 SHA=`107edf6a5a2abf31e718fdc8245b640ec251a5dab9a496f502e38bbf396ceacf`。
  v48→v49 仅三 identity fields 变化；历史 Profiles/CLOSED J1/terminals/live summaries 重放一致。v49 hidden、
  EXPERIMENTAL、ungranted、非 product-live。A1 dedicated `.sdlc/evidence/20260818-111558-image-only-v49-successor/`
  PASS（87/87 + PG 10/10），fast `.sdlc/evidence/20260818-111730-fast/` PASS；首轮 server stale V020 count
  negative evidence 保留于 `.sdlc/evidence/20260818-111759-server/`，修正后
  `.sdlc/evidence/20260818-113122-server/` PASS（20/13/388/285，0 failures/errors）。ticket 29 为当前
  Provider-zero preparation 前沿；OPEN=0、Goal live usage 0/1,500,000，未 apply/publish/deploy、未 commit/push。
- 2026-08-18 **IOPA-P1-R12 v49 bounded mixed-field correction 已 `automated_verified/resolved`**：ticket 27
  仅允许 approved mixed primary + OBSERVE + 全部 Prompt-covered canonical detail set 重试；retry task 只带 fixed
  codes。相同 exact set 第三次 rejected attempt 先落账/结算再熔断，异 set 隔离但不能绕过总 hard cap；第三次落账后
  crash/replay 由 replacement 零 Provider 调用终止。正向 scripted tracer 到 `REVIEW_REQUIRED`，Candidate unapplied、
  StaticSchema count 不变；v47/v48 semantics 重放不变。A1 gate
  `.sdlc/evidence/20260818-110846-image-only-v49-correction/` PASS（verifier 2/2、inference 47/47、Testcontainers
  PostgreSQL 11/11）；v49 Profile/Prompt15 absent、OPEN=0、Goal live usage 0/1,500,000。ticket 28 为当前
  Provider-zero 前沿；未 apply/publish/deploy、未 commit/push。
- 2026-08-18 **IOPA-P1-R11 v49 bounded rejection envelope 已 `automated_verified/resolved`**：ticket 26 已将
  primary/earliestStage/canonical detail/count 贯通 strict codec、worker、attempt、V020 PostgreSQL ledger、terminal、
  API/OpenAPI 与 Web；terminal 只使用 primary，detail 仅为 payload-free fixed-code/count。Testcontainers 覆盖
  malformed rejection、append-before-terminal、SETTLED/0 unsettled 与 attempt 落账后 lease crash/replay，replacement
  worker 不再签发 Provider permit。A1 gate `.sdlc/evidence/20260818-105626-image-only-v49-envelope/` PASS（inference
  49/49、PG/API 6/6、Web 77/77、独立 verifier）；v48 hash 不变，v49 Profile/Prompt15 absent，OPEN=0，Provider/
  key usage=0。ticket 27 为当前 Provider-zero 前沿；Goal live usage 仍 0/1,500,000，未 apply/publish/deploy、未
  commit/push。
- 2026-08-18 **IOPA-P1-R10 v49 generic fallback provenance 已 `automated_verified/resolved`，持续 Goal 已启动**：
  ticket 25 的 successor-only classifier 与 13 个 payload-free fixtures 已落地；known mixed 产生固定 primary +
  canonical 2..7 detail set，unknown 只产生 unclassified，v48 legacy 与合法输入行为不变。A1 gate
  `.sdlc/evidence/20260818-101738-image-only-v49-provenance/` PASS（Java 46/46、独立 taxonomy/identity replay、
  v49 Profile absent、OPEN=0、Provider/key usage=0）；fast `.sdlc/evidence/20260818-102357-fast/` PASS。
  ADR-0041 记录 successor-only typed envelope，ADR-0042 记录所有者于 `2026-08-18T10:23:09+08:00` 启动持续
  Goal 并批准按推荐方案实例化更窄 exact J1；新 Goal 真实调用累计 model-token hard cap=1,500,000，不能作为
  wildcard permit。ticket 26 为当前 Provider-zero 前沿；v49 尚未创建，未 apply/publish/deploy、未 commit/push。
- 2026-08-18 **IOPA-P1-R09 v49 mixed-region recovery contract 已 `human_accepted/resolved`**：所有者于
  `2026-08-18T09:54:40+08:00` 对 ticket 24 的 Q1–Q4 全部按推荐批准；approved delta =
  `specs/changes/20260818-image-only-v49-mixed-region-fallback-successor.md`。known mixed 使用
  `VISUAL_GROUNDING_REGION_FIELDS_INVALID` + 七码 closed enum 中 2..7 个成员的 canonical detail set，无法安全
  分类使用 `VISUAL_GROUNDING_REGION_UNCLASSIFIED` 且无 detail；相同 mixed set 第三次 rejected attempt 落账后
  breaker，unclassified/未知/未列首次落账后 terminal。future exact successor =
  `dashscope-qwen38-max-product-v49-hybrid-generic` / pipeline 4.31 / Prompt 15；相对 v48 Profile 只改三个 identity
  fields，最终 SHA 从实际 resource 计算。fresh diagnostic caps≤1 run/5 calls/100,000 tokens/¥3、每 run 5 calls/¥3、
  ≤2h，且仍需 fresh exact paid-live J1。本次只落盘 source decision 与 Provider-zero contract；v49 尚未创建，ticket
  25 为当前前沿，Provider usage 新增=0、OPEN authorization=0，未读取/输出 Key、未 apply/publish/deploy、未
  commit/push。
- 2026-08-18 **IOPA-P1-R08 v48 diagnostic 已 `TERMINAL_CLOSED/FAILED`，禁止自动重试**：所有者签发 exact J1
  `20260818-iopa-v48-diagnostic-4e1f41b7`，绑定 fresh cycle/profile/manifest/evaluator/normalization、旧失败单图与
  1 run/5 calls/100,000 tokens/¥3/2h。唯一 run `049b109a-d6ce-4dc2-a943-1a6ed651c960` 的首次 OBSERVE 以
  generic `VISUAL_GROUNDING_REGION_INVALID` rejected；该 code 不在 Prompt 14 field-specific allowlist，因此未签发
  第 2 次 reservation/call。实际 1 call / 13,394 tokens / ¥0.218928 / 0 unsettled；0 Candidate/review pack、
  credit=0、next stage=false、未 apply/publish。live A1 evidence =
  `.sdlc/evidence/20260818-084802-image-only-v48-successor-diagnostic-live/`；authorization/ledger 均 CLOSED，OPEN=0；
  terminal record = `plans/image-only-profile-successor-diagnostics/4e1f41b7-7c42-40d8-afd6-9fe3a35cc54d-terminal.json`。
  CLOSED 后 Provider-zero replay `.sdlc/evidence/20260818-085324-image-only-v48-successor/` PASS（verifier 4/4、
  inference 86/86、PG 9/9），未新增调用。P1 scoring/P1-02/P1-03 不解锁；再次恢复须新 source ticket/new
  identities/new cycle/fresh J1，不得 patch/rerun v48。未读取/输出 Key、未 commit/push。
- 2026-08-18 **IOPA-P1-R03..R07 v48 region-recovery pre-live checkpoint 为 `automated_verified`**：所有者对
  ticket 18 的 Q1–Q4 回复“全部按推荐”，approved delta =
  `specs/changes/20260818-image-only-v48-region-recovery-successor.md`，tickets 18–22 resolved。新增 hidden
  `dashscope-qwen38-max-product-v48-hybrid-generic`，canonical SHA =
  `22f40ef4c865e11778eef4558c20c383e6611e068d8d08be0d080650074d4470`；相对 v47 只改 profileId、pipeline
  4.30、Prompt 14。七个 field-specific region code 与显式 correction allowlist 已贯通；generic/未知/未列/混合
  code 首次落账即停，allowlist 同码第三次熔断。scripted-provider + Testcontainers 证明纠正成功可到
  `REVIEW_REQUIRED`，Candidate 仍 unapplied、StaticSchema unpublished。fresh cycle =
  `4e1f41b7-7c42-40d8-afd6-9fe3a35cc54d`，normalization =
  `/1.0:052e77dabb723f07e76b092e3da8afe1b5a56f7a40dc094451c18c42ee4f9aaa`，manifest =
  `/1.0:7d14e0b85bf07fc67ae20f0399e00be17a86511822fdae15d180a0a1171ecea7`，evaluator =
  `/1.0:b2167261ae9d1e3775c91d06d90c57c47c16284d11b685e81aa5073de655f37e`；prepared auth id =
  `20260818-iopa-v48-diagnostic-4e1f41b7`，caps=1 run/5 calls/100,000 tokens/¥3/≤2h，状态 `PENDING_J1`。
  A1 `.sdlc/evidence/20260818-015529-image-only-v48-successor/` PASS（inference 86/86、PG 9/9、独立 verifier
  3/3）；fast `.sdlc/evidence/20260818-015856-fast/` PASS；server
  `.sdlc/evidence/20260818-015929-server/` A1 PASS（20/13/377/276，0 failures/errors；live 默认 skip）。Provider
  attempts/reservations/cost/key reads=0、OPEN=0。该 checkpoint 当时未读取 Key、未调用 Provider；后续 live
  immutable terminal 见上一条。
- 2026-08-17 **IOPA-P1-R02 v47 successor diagnostic 已 `TERMINAL_CLOSED/FAILED`，禁止自动重试**：所有者签发
  exact J1 `20260817-iopa-v47-diagnostic-4ae94545`，只绑定 cycle
  `4ae94545-2c95-41dc-934e-1661aeb6c121`、旧失败 artifact 与 1 run/5 calls/100,000 tokens/¥3/2h。唯一 run
  `4cc392fa-9bc4-4594-a426-9cc785809c05` 在 OBSERVE 连续三次以
  `VISUAL_GROUNDING_REGION_INVALID` rejected；pipeline 4.29 在第三次落账后熔断，未签发第 4 次 reservation/call。
  实际 3 calls / 39,665 tokens / ¥0.661812 / 0 unsettled；0 Candidate、diagnostic PASS=false、certification
  credit=0、next stage=false、未 apply/publish。A1 evidence =
  `.sdlc/evidence/20260817-210024-image-only-successor-diagnostic-live/`；authorization、ledger 均 CLOSED，OPEN=0；
  terminal record =
  `plans/image-only-profile-successor-diagnostics/4ae94545-2c95-41dc-934e-1661aeb6c121-terminal.json`。P1-R03/R04 与
  P1-02/P1-03 不解锁；CLOSED 后 Provider-zero replay
  `.sdlc/evidence/20260817-210606-image-only-successor/` PASS，未新增调用。再次恢复必须开新 source ticket/new
  identity/new cycle，并另取 exact J1，不得 patch/rerun v47。
- 2026-08-17 **IOPA-P1 successor Provider-zero 已 `automated_verified`（pre-live checkpoint）**：所有者对
  ticket 17 的 Q1–Q3 回复“全部按推荐”，ticket 已 `resolved`，approved delta =
  `specs/changes/20260817-image-only-v47-certification-successor.md`。新增 immutable
  `dashscope-qwen38-max-product-v47-hybrid-generic`，canonical SHA =
  `a9fe98e1cfa4b7cc126db1f74601fdebe60526a1c999924daf189ed5f1ac5eb0`；相对 v46 只改 profileId、pipeline
  4.29、Prompt 13。Prompt 限制每组≤3 representative ITEM、regions/elements 各≤32；pipeline 在同 run/stage/code
  第 3 次 rejected attempt 落账后熔断，且 v46 新 grant 被 `PROFILE_CERTIFICATION_PROFILE_SUPERSEDED` 拒绝。
  非计分 diagnostic cycle = `4ae94545-2c95-41dc-934e-1661aeb6c121`，manifest =
  ...
  caps=1 run/5 calls/100,000 tokens/¥3/≤2h、certification credit=0。A1 gate
  `.sdlc/evidence/20260817-204336-image-only-successor/` PASS，Testcontainers PostgreSQL V019、exact identity replay、
  breaker/历史回归全绿；该 checkpoint 的 Provider attempts/reservations/cost/key reads=0、OPEN authorization=0，
  Candidate 未 apply、StaticSchema 未发布。prepared auth id=`20260817-iopa-v47-diagnostic-4ae94545` 当时为
  `PENDING_J1`；其后 exact live 终态见上一条。扩大验证 fast
  `.sdlc/evidence/20260817-204612-fast/` 与 server `.sdlc/evidence/20260817-204629-server/` 均 PASS；server 为
  20/13/369/272 tests、0 failures/errors，未新增外部调用。
- 2026-08-17 **IOPA-P1-01 已 `terminal_failed`，exact J1/ledger 均 CLOSED，P1-02 未解锁**：所有者签发
  `20260817-iopa-canary5-c3bde304`，绑定 cycle `c3bde304-b0b2-43f8-ab7e-16896ff04aed`、v46 exact SHA、fresh
  manifest、5 张 USER_PROVIDED+ORDINARY_DESIGN PNG、总 cap 5 runs / 60 calls / 500,000 tokens / ¥10 / 4h，
  每 run cap 12 calls / ¥6。更新后 Provider-zero preflight
  `.sdlc/evidence/20260817-181159-image-only-p1-preflight/` 与 frozen OCR canary
  `.sdlc/evidence/20260817-181056-document-vision/` 绿色。第一次 live 入口
  `.sdlc/evidence/20260817-181304-image-only-p1-live/` 在 stage/open/egress 前因 180s OCR timeout 不符合 60s
  合同而拒绝，usage=0；报告并修正后，唯一实际批次
  `.sdlc/evidence/20260817-181409-image-only-p1-live/` 启动 2/5：case 1 经 5 calls 到达
  `REVIEW_REQUIRED`，case 2 用满 12 calls 后以 `VISUAL_GROUNDING_PARENT_CONTAINMENT_INVALID` 失败，其余 3
  未启动。终态 17 calls / 301,409 tokens / ¥6.338772 / 0 unsettled，Candidate applied=false、StaticSchema
  published=false；J1 于 `2026-08-17T10:49:06.053985Z` CLOSED。payload-free terminal record =
  `plans/image-only-certification-cycles/c3bde304-b0b2-43f8-ab7e-16896ff04aed-canary5-terminal.json`；仅 case 1
  review pack 留在 `.scratch/image-only-certification-reviews/20260817-iopa-canary5-c3bde304/`，待所有者查看但不得
  apply。CLOSED 后完整 Provider-zero server gate `.sdlc/evidence/20260817-185255-server/` PASS（20/13/362/267，
  0 failures/errors）。按既定 5/5 gate，禁止 patch/rerun，P1-02/P1-03 不解锁；任何改变须按 Blueprint 附录 A
  开新票。
- 2026-08-17 **IMAGE_ONLY production admission P0-01..05 已 `automated_verified`**：创建 hidden
  ...
  `grantsProviderEgress=false`。历史下一入口 IOPA-P1-01 已执行并失败；ticket 17 已 resolved，当前恢复入口为
  v47 non-scoring diagnostic 的 fresh exact J1，P2 仍可并行安全推进。
