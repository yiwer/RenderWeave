# RenderWeave Template v1 历史执行卡（T01–T210 / F123–F194）

> 归档自 plans/renderweave-template-v1-plan.md（2026-09-03）。执行卡保留为历史事实，不再驱动工作；新票见 .scratch/renderweave-template-v1-implementation/issues/（从 T211 起）。
...
- 决策：冻结 Domain Services class gate 要求 exact domain-service + PostgreSQL integration target，以及
  `java-domain-authority`、`transactional-integration-replayer` 两个角色。T127 明确没有数据库执行且
  `recordIssuanceAllowed=false`，所以本票先补齐类级 preissuance replay，不直接 append 12+12 formal records。
- Transaction seam：新增 app test-scope replay，执行真实 `PostgresAssetPersistence`；固定 create 成功后必须原子形成
  aggregate/content/audit/idempotency/capacity 五项观察，幂等 replay/conflict 不增加写，末步 duplicate idempotency key
  失败必须回滚此前 aggregate/content/audit 行。只允许 Testcontainers PostgreSQL，不允许 H2/SQLite/mock 替代。
- Exact target：冻结 `postgres:16-alpine` tag 的预加载 image digest，绑定 T127 component target、产品与迁移 Git blob、
  12 Case + 12 Oracle assigned candidate subset digest、formal registry 46/46 零 Domain 记录及两个 executor manifests。
  Python 独立 verifier 只在完整闭包通过后报告 `preissuanceReady=true`，不宣称 class executable。
- 允许影响：T128 tracker/map/plan、专用 app integration test、Domain Services target/executor manifests/materializer、
  独立 verifier、专用 gate 与必要 evidence；不修改用户既有 dirty 文件。
- 禁止影响：formal Case/Oracle registry、central catalog/历史 static evidence、API/OpenAPI/migration/Web/产品语义、
...
- 决策：Rendering 在 HTTP content encoding 与严格 RenderInput admission 的九个真实 reservation point 派生 observation，
  统一调用 T130 `DesignInputExpressionCapacityAuthority`；阈值、比较器与 terminal 不再由 Rendering product path 复制。
- 深 seam：Template configuration 暴露 canonical bean；App controller 与 Rendering evaluator 注入同一 authority；parser
  以 authority-backed budget 保持 strict UTF-8/JSON、first-error 与 pre-allocation candidate reservation，其他内部 JSON
  consumers 继续使用固定 budget。authority `Invalid` fail closed 为 `RENDER_INTERNAL_ERROR`。
- TDD/target：recording/rejecting proof 先 RED，覆盖 contentEncoding、utf8Bytes、jsonDepth、objectMembers、arrayItems、
  totalValuesAndContainers、stringUtf8Bytes、numberTokenBytes、customValueEntries；实现提交后新建不可变 v3 target，声明
  wired 33/65、remaining 32，v1/v2 bytes 不变。
- Gates：focused Rendering/App → component → `template` → `fast` → sequential `server` → Goal `full` → resolution
  `fast`；Maven 串行、精确 staging，formal registry 保持 58/58。
- 禁止影响：problems/expression/geometry 其余 32 轴、class manifests/195 formal records、其他 execution class、Renderer/
  Profile/native build、provider/API Key/真实数据/生产/J1/A3/READY、用户 360 项 dirty work 与 stash；
  `BUILD_NOT_AUTHORIZED` 保持。
- Results：实现 revision `9d316fc419314e017812ca08b4548584723a75b2` 完成九轴共享 authority 接线与 12 项
  reservation proof；target revision `4d4d091adcd39dc9a3280d12024858b54cfae24d` 冻结 v3（13923 bytes，SHA-256
  `7f582b7b3efa340b87a2e5e36f3d944d861d362a20f87cca0c5eb6f156aa6d92`），v1/v2 bytes 不变。component
  `.sdlc/evidence/20260826-175619-design-input-expression-capacity/` 为 Java/TypeScript 195/195、2646 checks，wired
  33/65、remaining 32，formal registry 58/58，class preissuance/issuance/executable 均 false。
- Gates：`template` `.sdlc/evidence/20260826-175817-template/`、`fast`
  `.sdlc/evidence/20260826-175914-fast/`、顺序 `server` `.sdlc/evidence/20260826-180004-server/` 全绿；发布级
  `full` `.sdlc/evidence/20260826-181806-full/` 为 17/17 steps、1744.369 秒，Node 24 Web 32 files / 251 tests、
  Chromium 25 passed + 1 controlled skip、inference replay 1/1 全绿，provider attempts/API Key reads/reservations/cost 为 0。
- Resolution：tracker 收口后 `fast` `.sdlc/evidence/20260826-184921-fast/` 通过（8/8 package reactor + Node 24
  typecheck）。
- Boundary：用户 360 项 dirty work 指纹 `4bddd1c955a4b4f55d984f3febd551742dd6f63c` 与 stash
...
- 状态：`automated_verified`；Blocked by T134（resolved）。
- 决策：只接线 `expression.astNodesPerExpression` 与 `expression.astNodesTotal`；decimal 七轴和 geometry 十轴继续
  pending。TDD seam 沿用已冻结的 `DesignDslAuthority.admit(rawUtf8)`，不新增第二条 admission Interface。
- 深 module：把 exact Expression grammar parser 与 immutable AST 派生值收进 Template-owned
  `DesignSemanticAuthority`；admission 与 Rendering 消费同一实现，删除 Rendering 的重复 parser/AST 与硬编码 AST
  limit。`Interpreted` 一次交付 canonical Design semantic tree 与 definitionId→AST map。
- Reservation：`ExpressionDefinitionCapacityBudget` 保持单次 DesignDSL request-local；每个 parser node 在 allocation 前
  先验证 per-expression candidate，再验证全 DSL candidate，两者 Accepted 后才提交计数。未使用/静态不可达
  Expression 同样按 authored definition 顺序解析并计数；authority reject/invalid/throw 与 checked overflow 均在
  exact source pointer 零写退出。
- TDD/target：真实 admission recording/rejecting RED → 最小 parser ownership/AST reservation GREEN → Expression engine
  与 canonical 211-vector regression；实现后新建不可变 v6 target，声明 wired 48/65、remaining 17，v1–v5 bytes
  不变。
- Gates：focused Template/Rendering → component → `template` → `fast` → sequential `server` → Goal `full` →
  resolution `fast`；Maven 串行、精确 staging，formal registry 保持 58/58。
- 禁止影响：decimal/geometry 其余 17 轴、class manifests/195 formal records、其他 execution class、Renderer/Profile/
  独立 native build、provider/API Key/真实数据/生产/J1/A3/READY、用户 360 项 dirty work 与 stash；
  `BUILD_NOT_AUTHORIZED` 保持。
- Results：实现 revision `91da8d56be811139835a845f5d5313682cf4a6d9` 已完成 Template-owned parser/immutable
  AST、逐 node 原子 reservation 与 Rendering 单一 AST 消费 seam；focused 17/17、Expression engine 28/28、
  Template 114/114、Rendering 131/131 与 canonical Java/Python 211/211 全绿。
- Target/component：target revision `93ba7a30dfd1000dafc6a600a30f816d2c27a0d0` 冻结 v6（29,157 bytes，
  SHA-256 `5927679899d4c0b2c12159a7c61734b7cd6882190aa2812d00ef1dd21c8d878c`），v1–v5 bytes 不变；
  `.sdlc/evidence/20260826-225802-design-input-expression-capacity/` 为 195/195、2,684 independent checks、
  53 个 bound artifacts 零 mismatch，wired 48/65、remaining 17，formal registry 58/58，recordsIssued=0。
- Gates：`template` `.sdlc/evidence/20260826-225949-template/`、`fast`
  `.sdlc/evidence/20260826-230044-fast/`、顺序 `server` `.sdlc/evidence/20260826-230121-server/` 全绿；发布级
  `full` `.sdlc/evidence/20260826-231827-full/` 为 17/17 steps、1,575.495 秒，clean reactor 全绿，Node 24 Web
  32 files / 251 tests、runtime/R0/R1/P0、Chromium 25 passed + 1 controlled skip、Draft 与 inference replay E2E
  均通过。
...
- 状态：`resolved / automated_verified / unclaimed`；Blocked by T197（resolved）。
- authority：Ticket 19 `RW-T19-S7-109 / RW-T19-S9-017 / RW-T19-S9-019` 与 cap-059 固定
  `geometry.bleedMmPerSideMax=100`、MAX_INCLUSIVE / CANONICAL_DECIMAL；terminal 为
  `DESIGN_PROPERTY_CONSTRAINT_INVALID / DESIGN_PROPERTY_AND_AGGREGATE_VALIDATION`，public stage
  TEMPLATE_CLOSURE，reservation point 为 persisted write/Evaluation 前，zero boundary 为
  ZERO_WRITE_AND_DOWNSTREAM。
- seam：只深化既有 `DesignDslAuthority.admit(rawUtf8)`；可选 Canvas bleed 缺失时不观察，存在时按
  `topMm -> rightMm -> bottomMm -> leftMm`，每边共享 canonical value 并依次进入 min→max；任一失败立即停止，
  不新增 Interface 或第二 authority。
- TDD：先提升现有真实 Canvas 公共 seam 断言并取得 maximum observation 缺失 RED；再覆盖四边 below/at/
  scale-64 above、canonical decimal、minimum-before-maximum fail-fast 与 authority reject/invalid/throw 的 exact GREEN。
- target/gates：冻结 immutable component target v11，目标 wired 59/65、remaining geometry 6；执行 focused
  Template、component Java/TypeScript、canonical vectors、`template`、`fast`。无 app wiring，server/full 复用最近
  未变输入的绿色证据。
- boundary：不接线 cap-060–065，不发行 formal records/class manifests，不运行 Renderer/Profile/provider/API Key/
  真实数据/生产/J1/A3/READY，不改用户 dirty work/stash，不 push/tag/PR。
- 实现/证据：实现基线 `ca1a0b55fc008a55c5dfbc94437ba28aabcfe141`；immutable v11 target SHA-256
  `957abb68737d22ccbb9dfa8d4007135ee018b08c3e920e6df04c7653a1c94d90`、21391 bytes。public seam RED 后
  focused 30/30、Template module 153/153；component `.sdlc/evidence/20260829-1948-template-t198-component/`
  Java primary 195/195（A1）+ TypeScript independent 195/195、2692 checks（A2）；`template`
...
- 状态：`resolved / automated_verified / unclaimed`；Blocked by T200（resolved）。
- authority：Ticket 19 `RW-T19-S7-111 / RW-T19-S9-017 / RW-T19-S9-019` 与 cap-062 固定
  `geometry.fontSizePtMax=4096`、MAX_INCLUSIVE / CANONICAL_DECIMAL；terminal 为
  `DESIGN_PROPERTY_CONSTRAINT_INVALID / DESIGN_PROPERTY_AND_AGGREGATE_VALIDATION`，public stage
  TEMPLATE_CLOSURE，reservation point 为 persisted write/Evaluation 前，zero boundary 为
  ZERO_WRITE_AND_DOWNSTREAM。
- seam/scope：只深化 `DesignDslAuthority.admit(rawUtf8)` 与同一 Template-owned capacity authority；同一
  `text.runs[*].fontSizePt` 只 parse 一次并复用 canonical value，按 cap-061 minimum→cap-062 maximum 后再进入
  下一 authored Run。Binding overlay 继续复用同一 re-admission seam；cap-063–065 保持 deferred。
- TDD：先提升真实 Text 公共 seam 的两 Run observation 顺序并取得 cap-062 缺失 RED，再覆盖 below/at/
  scale-64 above、canonical decimal、exact pointer、minimum-before-maximum 与 authority reject/invalid/throw。
- target/gates：成功后冻结 immutable component target v14，目标 wired 62/65、remaining geometry 3；执行
  focused Template、Template module、component Java/TypeScript、canonical vectors、`template`、`fast`。无 app
  wiring，不重复 server/full。
- boundary：不接线 cap-063–065，不发行 formal records/class manifests，不运行 Renderer/Profile/provider/API Key/
  真实数据/生产/J1/A3/READY，不改用户 dirty work/stash，不 push/tag/PR。claim 时 A0、J0 pending。
- 实现/证据：实现基线 `1f8fc3596f43d6844ed9cabde3ef23f9d4cd3a7b`；每个 Text Run 复用一次 parsed
  decimal/canonical value 并按 cap-061→cap-062 fail-fast，below/at/scale-64 above 与 injected authority 故障身份
  已覆盖。immutable v14 target SHA-256
  `ab0c270c754312ca4c03a52061d6b5a7147cb894179a9a99a8a5e6232bf471b8`、21522 bytes；component
  `.sdlc/evidence/20260829-204006-template-t201-component/` Java primary 195/195（A1）+ TypeScript independent
  195/195、2692 checks（A2），wired 62/65、remaining geometry 3、formal records 0。focused 34/34、Template
