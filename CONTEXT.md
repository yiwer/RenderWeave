...
| Asset acceptance kernel | `renderweave-asset` 中首个真实产品原子，通过唯一 `AssetAcceptanceAuthority.admit(rawBytes, kind)` 对静态 PNG/JPEG/WebP 与单 face、non-variable、monochrome outline TTF/OTF 执行 strict magic/结构/CRC/完整解码/字体表解析与 64MiB/32MiB/2 万 px/1 亿像素预算，输出 TechnicalDescriptor/sha256/byteLength。 | 不是 Asset create 纵切、canonical sRGB ICC 等值接受、动画/变量/彩色字体、Asset 聚合、Blob/S3、Resolver/lease 或 acceptance Profile available 资格。 |
