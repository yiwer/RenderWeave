# 实现 definite Stack 三 FILL 第二 max freeze 末项 inactive min 子闭包

Type: task
Status: resolved / automated_verified
Claimed by: Codex `/root`（single-writer）
Blocked by: 23, 25, 26, 33, 38, 43, 55, 56, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78（均已 resolved）

## Question

Ticket 10 §3/§7 冻结了 multiple Stack main-FILL 的 weighted iterative min/max water filling。T76 已允许恰好
三项、首轮唯一 active max、一次重分配后恰好一个第二 max active、最后一项无界的固定两次 freeze。如何接受
最后一项携带一个在重分配和最终 exact remainder 下均 inactive 的 min-only bound，同时不开放末项 max/mixed、
第三次 freeze、一般循环或 Profile residual tolerance？
...
2. **固定准入子集**：owning definite Stack 恰好有三个 main-axis FILL；三项 weight 均 positive；第一轮恰好一个
   ...
   bound active，唯一末项只携带 finite/nonnegative min-only bound，且在该次重分配 share 下 inactive。
3. **末项只复核、不再 freeze**：第二 max freeze 后继续以既有固定顺序计算唯一
   `lastShare = remaining - firstMax - secondMax`；只有 `lastShare >= terminalMin` 才提交 staged allocations。
   不做新的 division、第三次 freeze、循环、epsilon/tolerance 或 justify 特判；末项 min 不参与重新分配。
...
5. **共同语料与 TDD**：shared vector/verifier identity 升级为 `/42`。保留 T76 五个 unbounded positives，并在
   同样的 active-first、active-middle、active-last、COLUMN 与 cross-HUG 形状上新增 terminal inactive-min
   positives；把既有 terminal-min negative 转 positive，新增 terminal max-only negative。能力值新增
   `OR_EXACT_THREE_FILL_SECOND_MAX_FREEZE_TERMINAL_INACTIVE_MIN`；目标 187 laid-out + 16 unsupported、203
   cases、607 checks，fixture `/3` bytes 不变。
6. **固定能力边界**：terminal max/mixed/active min、首轮多个 active、第一次重分配后多个 active、首个 active
   min、second min/mixed、active-min overflow、second-min sum overflow、四项及以上 active-bound FILL、第三次
...
- TDD：先只改 shared `/42` vectors/identity，让 Rust primary 与 Python independent verifier 在同一首个新增
  terminal-min case 共同 RED；再分别实现严格控制流并达到 exact-bit GREEN。
...
- shared `/42` 由 Rust primary 与 Python independent verifier 在同一首个 terminal inactive-min 转正 case
  共同 RED 后，分别实现并达到 187 laid-out + 16 unsupported、203/203 cases、607 checks；vector SHA-256
  为 `871b3fe96fec15766298a9a155f314fa958fc9141231864b51e4d6b5fca5002f`，fixture `/3` SHA-256 保持
...
- `stack_main_fill_allocations` 只在 exactly-three、首轮唯一 active max-only、第一次重分配后唯一 second
  max-only active、唯一末项仅携带 finite/nonnegative 且始终 inactive 的 min-only 时复用固定两次 max
  freeze 与 exact remainder；final share 不低于 terminal min 才提交。能力值新增
  `OR_EXACT_THREE_FILL_SECOND_MAX_FREEZE_TERMINAL_INACTIVE_MIN`；不新增 division、第三次 freeze、循环或
  tolerance。
...
- 诚实边界保持不变：Profile `NOT_REGISTERED`、certification `NOT_CERTIFIED`、world transform/scene/raster
  `ABSENT`、daemon output `UNWIRED`；terminal max/mixed/active min、多个 active、four-or-more、第三次 freeze
  与一般 water filling 继续 fail closed。未推进 A3/J1/READY，未 push/tag/PR。
- 本状态更新后的 resolution `fast` `.sdlc/evidence/20260823-222807-fast/` 3 steps 也均 exit 0
  （A1，12.410 秒）。
