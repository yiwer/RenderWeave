...
- [激活 Template 最终产品路由与浏览器验收](issues/126-activate-template-product-routes.md) —
  **resolved / automated_verified**；已开放 `/templates` list/new/editor、一级产品导航与 Editor 返回目录路径，并以只访问
  正式 URL 的 Playwright journey 验收真实页面、创建/打开闭环及 E6 Authoritative Preview。focused 23/23、正式 journey
  2/2、clean Node 24 Web 250、`web`、affected/resolution `fast` 与 17-step Goal `full` 均绿；实现 revision
...
- [物化 Domain Services 容量一致性产品执行目标](issues/127-domain-services-capacity-conformance-target.md) —
  **resolved / automated_verified**；Domain Services 的 4 个 Asset 容量轴与 12 个 frozen fixture 已绑定唯一生产
  ...
  `d73f5650`；Asset 97/97、Java/Python 12/12、正式 `asset`/`fast`/顺序 `server`/发布级 `full` 均绿，formal
...
- [接线 DesignDSL semantics 十五轴产品容量](issues/131-wire-design-dsl-semantic-capacity.md) —
  **resolved / automated_verified**；在真正 DesignDSL admission 内以 request-local preflight 把 authored tree、
  definition、Binding、Text run、Grid、vector、TemplateUse fill 与 literal list 十五轴接到同一 authority。component
  target v2 冻结并完成 195/195、2608-check 双重 replay，wired 由 9/65 推进至 24/65；v1 不可变，剩余 41 轴与
  class manifests/issuance/executable 继续 pending。
- [接线 RenderInput 九轴产品容量](issues/132-wire-render-input-capacity.md) —
  **resolved / automated_verified**；HTTP content encoding、Rendering strict JSON 八项计数与 custom-value entry 的九个
...
- [T200 — 接线 font-size pt exclusive-min 产品容量](issues/200-wire-font-size-pt-exclusive-min-capacity.md)
  ...
  `geometry.fontSizePtExclusiveMin=0` 已按 authored Run order 覆盖每个 Text Run `fontSizePt`；Java/TypeScript
  195/195、Template 168/168、Java/Python 211/211、`template`/`fast` 全绿，wired 61/65、remaining geometry 4、
  formal records 0。A3 无，J0 pending、J1 未批准；claim 已释放，下一 unblocked frontier 为 cap-062
  `geometry.fontSizePtMax`，尚未登记或 claim。
...
- [T197 — 接线 Canvas bleed 每边 minimum 产品容量](issues/197-wire-canvas-bleed-minimum-capacity.md)
  ...
  `geometry.bleedMmPerSideMin=0` 已按四边 frozen wire order 经既有 public admission seam 接线；Java/TypeScript
  195/195、Template Java/Python 211/211、`template`/`fast` 全绿，wired 58/65、remaining geometry 7。A3 无，
  J0 pending、J1 未批准；cap-059–065 与 formal issuance/class manifests 保持 pending。下一 unblocked frontier
  为 cap-059 `geometry.bleedMmPerSideMax`，尚未登记或 claim。
