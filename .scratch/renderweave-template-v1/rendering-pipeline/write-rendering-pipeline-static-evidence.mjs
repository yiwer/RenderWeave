...
const targetPath = "rendering-pipeline/generator-target-manifest-v1.json";
const primaryEntrypoint = "rendering-pipeline/generate-rendering-pipeline-fixtures.mjs";
const independentEntrypoint = "rendering-pipeline/validate_rendering_pipeline_fixtures_independent.py";
...
writeJson("rendering-pipeline/primary-result-v1.json", primaryResult);
writeJson("rendering-pipeline/independent-result-v1.json", independentResult);
...
if (!readiness) throw new Error("RENDERING_PIPELINE readiness missing");
...
])) throw new Error("RENDERING_PIPELINE blocker frontier drifted");
...
if (axes.length !== 52) throw new Error(`RENDERING_PIPELINE axis count drifted: ${axes.length}`);

const evidence = {
  evidenceVersion: "renderweave-rendering-pipeline-fixture-static-a2/1.0",
  evidenceId: "RENDERING_PIPELINE_FIXTURE_STATIC_A2::2026-08-17::000001",
  status: "PASS",
  grade: "A2_INDEPENDENT_STATIC_REPLAY",
  scope: "RENDERING_PIPELINE safe-baseline, closed observation-adapter, deterministic generator, 157 fixture artifacts, and 156 isolated capacity-guard inputs only; no closure freeze, capability state, Asset admission or resolution, Evaluator, sealer, Renderer Command handoff, Engine, or terminal output was executed.",
  specRevision: {
  ...
  authorityBindings: {
    ...
    baseline: artifact("rendering-pipeline/baseline-v1.json"),
    fixtureContract: artifact("rendering-pipeline/fixture-contract-v1.json"),
    observationAdapter: artifact("rendering-pipeline/observation-adapter-v1.json"),
    scenarioCatalog: artifact("rendering-pipeline/capacity-scenarios-v1.json"),
    generatorGoldens: artifact("rendering-pipeline/generator-goldens-v1.json"),
    generatorTarget: artifact(targetPath),
    generatorImplementation: artifact("rendering-pipeline/generator-implementation-manifest-v1.json"),
  },
  fixtureInventory: {
    fixtureCount: target.fixtureArtifacts.length,
    capacityFixtureCount: 156,
    namedFixtureCount: 1,
  ...
  replay: {
    primary: {
      result: artifact("rendering-pipeline/primary-result-v1.json"),
    ...
    independent: {
      result: artifact("rendering-pipeline/independent-result-v1.json"),
  ...
  findings: {
    renderingPipelineCapacityAxisCount: axes.length,
    ...
    guardSeamReason: "The isolated canonical-integer guard avoids fabricating giant closure, node, resource, document, deadline, or retention payloads while requiring each future exact product target to prove authoritative stage derivation and reuse the same limit-specific guard at the frozen reservation point.",
  },
  formalRegistryBoundary: {
    cases: { ...formalCases, recordCount: 46 },
    oracles: { ...formalOracles, recordCount: 46 },
    issuedRenderingCapacityCaseCount: 0,
    issuedRenderingCapacityOracleCount: 0,
  ...
  sideEffects: {
    ...
    closureFreezeInvocations: 0,
    capabilityStateInvocations: 0,
    assetResolverInvocations: 0,
...
writeJson("rendering-pipeline/rendering-pipeline-fixture-static-a2-2026-08-17.json", evidence);
process.stdout.write(`${JSON.stringify({
  status: evidence.status,
  evidence: artifact("rendering-pipeline/rendering-pipeline-fixture-static-a2-2026-08-17.json"),
