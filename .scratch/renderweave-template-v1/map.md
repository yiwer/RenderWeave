...
- Template 没有发布状态；metadata 位于 DesignDSL，每次 accepted save 追加不可变 revision；TemplateRef 只跟随 current，DELETED 是不可恢复终态。
...
- Expression 只可经显式输入使用封闭 Clock/Random Evaluation Capabilities；AssetResolver 是实际 asset occurrence 的独立内部解析合同，二者默认都不承诺跨次求值可重放。
- Asset Management 首版只接纳 IMAGE/FONT，以稳定 Asset、不可变内容版本、逻辑 current、scope 内 Blob 去重、标签目录和可恢复软删除运行；被引用 Asset 仍可经精确影响确认删除。
- 首个语义目标是有限二维画板生成一张或一组图片；主要作者是理解 StaticSchema、字段路径和表达式的技术型/低代码模板设计者。
...
- [从历史系统提取 DesignDSL 语义证据](issues/01-historical-design-dsl-evidence.md) — 历史系统证明精确 Schema 上下文、集中动态语义与静态 Scene 边界可行，也暴露了属性权威分散、生产 loop/kind 未闭合及无通用 datasource/图片 Renderer 的缺口。
...
- [划定 Template、Render 与 Asset 的限界上下文](issues/03-bounded-contexts-and-language.md) — 三个上下文以 TemplateSnapshot、AssetRef/ResolvedAsset 和 RenderDocument 单向交接；Evaluation 编译 DesignDSL 为 busbox-facing RenderDSL，Engine 内 LaidOutScene 再产生图片与权威预览。
...
- 循环、画板与输出基数明确后，再判断批量/变体生成是否形成独立领域，而非 Renderer 的一个选项。
- DesignDSL 与 RenderDSL/RenderDocument 版本策略明确后，再判断是否需要独立的迁移工具、conformance corpus 或 RenderEngine profile 认证工作。
