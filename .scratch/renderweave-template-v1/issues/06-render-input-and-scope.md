...
Repeat iterable 必须拥有静态封闭的 item 类型。标量项由Evaluator使用已验证item与原集合零基index构造对应精确`system-basic-text@v1`、`system-basic-decimal@v1`、`system-basic-date@v1`、`system-basic-time@v1`或`system-basic-boolean@v1` typed context；它们是正式Schema规格已预置的一等只读StaticSchema，每份都有必填`/index`与对应类型的必填`/value`。`/index`与loopIndex ValueSource表示同一值。只有来自已验证StaticSchema `array(items: reference)`的对象项才携带被引用的精确StaticSchemaRef且不注入index；任意expression/computed JSON object不能因结构相似而成为Schema context或子Template输入。嵌套Repeat只能按loopId显式读取当前或祖先frame，不能读取sibling/descendant。

每次 TemplateUse 调用创建隔离的子 invocation frame。子context只能由显式ContextSelector取得：选择与子Template永久StaticSchemaRef完全相同的invocation context、精确reference-typed subtree/loop item、完整system-basic scalar item context，或在子绑定`system-empty@v1`时选择显式empty context；禁止shape-based object构造和任意JSON注入。普通selector必须声明`ERROR | SKIP`，合法optional context运行时ABSENT时分别失败或完全跳过调用。传入typed subview携带exact StaticSchema证明并保持不可变，不经过JSON序列化或按形状重新验证；只有外部根RootDocument在每次Evaluation入口执行权威验证。

子 Custom 输入只能通过 TemplateUse 的显式 fill 列表按子当前 PUBLIC definitionId 赋值。每个 fill source 在父 TemplateUse 所在 invocation/loop 词法域中求值，因此循环内调用会逐 item 求值，然后把 typed value 复制进子 frame；子不能回读父 frame。省略 fill 或 fill source 得到 ABSENT 时使用子默认值，求值 ERROR 使 Evaluation 失败；重复 target、目标不存在/PRIVATE 或静态类型不兼容使父 Template 形成依赖 ERROR。外部 customValues 指向已删除或转为 PRIVATE 的根 definition 仍静默忽略，但持久化父 fill 指向同类变化必须经 TemplateRef 依赖重检使父 Template INVALID。子 PRIVATE definition 始终使用自身默认值，PUBLIC/PRIVATE 只约束 invocation 边界，两者在所属 Template 内均可被合法 definition/节点读取。子调用不继承父 custom、definition、RootDocument handle、loop frame、兄弟状态或通用系统 map；同一根 Evaluation 的实际 child invocation 只共享其 exact Profile 获准的请求级 CapabilityState 组件，仍须由子 Expression 显式声明 capability input，TemplateUse 不能配置 per-use allowlist。
...
ValueSource/Binding/Expression具体wire与完整类型系统、definition memoization、Repeat空集/缺失以及TemplateUse ContextSelector/ERROR-SKIP已分别由票据07、11、12冻结。capability exact合同已由票据14冻结，Evaluation fingerprint/result identity及未来Output cache的前置边界已由票据15冻结；API体验与数值容量仍由18/19票据继承以上不变量后收口。
