...
const targetPath = "design-input-expression/generator-target-manifest-v1.json";
const primaryEntrypoint = "design-input-expression/generate-design-input-expression-fixtures.mjs";
const independentEntrypoint = "design-input-expression/validate_design_input_expression_fixtures_independent.py";
...
writeJson("design-input-expression/primary-result-v1.json", primaryResult);
writeJson("design-input-expression/independent-result-v1.json", independentResult);
...
if (!readiness) throw new Error("DESIGN_INPUT_EXPRESSION readiness missing");
...
])) throw new Error("DESIGN_INPUT_EXPRESSION blocker frontier drifted");
...
if (axes.length !== 65) throw new Error(`DESIGN_INPUT_EXPRESSION axis count drifted: ${axes.length}`);

const evidence = {
  evidenceVersion: "renderweave-design-input-expression-fixture-static-a2/1.0",
  evidenceId: "DESIGN_INPUT_EXPRESSION_FIXTURE_STATIC_A2::2026-08-17::000001",
  status: "PASS",
  grade: "A2_INDEPENDENT_STATIC_REPLAY",
  scope: "DESIGN_INPUT_EXPRESSION safe-baseline, closed observation-adapter, deterministic generator, 196 fixture artifacts, and 195 isolated capacity-guard inputs only; no product parser, canonicalizer, semantic evaluator, RenderInput admission target, or terminal was executed.",
  specRevision: {
  ...
  authorityBindings: {
    ...
    baseline: artifact("design-input-expression/baseline-v1.json"),
    fixtureContract: artifact("design-input-expression/fixture-contract-v1.json"),
    observationAdapter: artifact("design-input-expression/observation-adapter-v1.json"),
    scenarioCatalog: artifact("design-input-expression/capacity-scenarios-v1.json"),
    generatorGoldens: artifact("design-input-expression/generator-goldens-v1.json"),
    generatorTarget: artifact(targetPath),
    generatorImplementation: artifact("design-input-expression/generator-implementation-manifest-v1.json"),
  },
  fixtureInventory: {
    fixtureCount: target.fixtureArtifacts.length,
    capacityFixtureCount: 195,
    namedFixtureCount: 1,
  ...
  replay: {
    primary: {
      result: artifact("design-input-expression/primary-result-v1.json"),
    ...
    independent: {
      result: artifact("design-input-expression/independent-result-v1.json"),
  ...
  findings: {
    designInputExpressionCapacityAxisCount: axes.length,
    ...
    guardSeamReason: "The isolated exact-string guard avoids allocating artificial 16-MiB, million-entry, or cross-axis-invalid payloads while requiring each future exact product target to prove authoritative value derivation and reuse the same limit-specific guard at the frozen reservation point.",
  },
  formalRegistryBoundary: {
    cases: { ...formalCases, recordCount: 46 },
    oracles: { ...formalOracles, recordCount: 46 },
    issuedDesignCapacityCaseCount: 0,
    issuedDesignCapacityOracleCount: 0,
  ...
  sideEffects: {
    ...
    parserInvocations: 0,
    semanticEvaluatorInvocations: 0,
    renderInputAdmissionInvocations: 0,
    networkAttempts: 0,
...
writeJson("design-input-expression/design-input-expression-fixture-static-a2-2026-08-17.json", evidence);
process.stdout.write(`${JSON.stringify({
  status: evidence.status,
  evidence: artifact("design-input-expression/design-input-expression-fixture-static-a2-2026-08-17.json"),
