...
public final class CandidateJsonCodec {
    ...
    private static String classifyDecodeFailure(Throwable failure) {
        ...
        if (mismatchedInput != null) {
            ...
            var slot = formatInvalidSlot(mismatchedInput);
    ...
    private static String formatInvalidDiagnostic(MismatchedInputException failure) {
        var slot = formatInvalidSlot(failure);
        return "CANDIDATE_DECODE_FORMAT_INVALID_" + (slot == null ? "OTHER" : slot);
    }
