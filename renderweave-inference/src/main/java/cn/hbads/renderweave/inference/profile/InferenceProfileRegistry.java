...
/** P4 registry deliberately exposes only a zero-network replay profile. */
public final class InferenceProfileRegistry {
    ...
    InferenceProfileRegistry(ClassLoader classLoader) {
        ...
        if (!"REPLAY".equals(replay.profile().provider()) || replay.profile().networkAllowed()) {
            throw new IllegalStateException("P4 replay profile must make network access impossible by contract");
        }
        profiles = Map.of(replay.profile().profileId(), replay);
    }
