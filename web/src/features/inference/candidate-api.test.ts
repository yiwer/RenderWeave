...
describe('live inference multipart request', () => {
  it('never sends retained files from an inactive input mode', async () => {
    ...
    await createLiveRunRequest(
      'dashscope-qwen37-plus-product-v44-hybrid-generic', 'JSON_ONLY', [image], [json], 'json-only', 250_000,
    );
    await createLiveRunRequest(
      'dashscope-qwen37-plus-product-v44-hybrid-generic', 'IMAGE_ONLY', [image], [json], 'image-only', 5_000_000,
    );
    ...
    await expect(secondMetadata.text()).resolves.toContain('"costLimitMicrosCny":5000000');
  });
  ...
  it('turns a non-JSON gateway 413 into a stable readable upload problem', async () => {
    ...
    await expect(createLiveRunRequest(
      'dashscope-qwen37-plus-product-v44-hybrid-generic', 'IMAGE_ONLY', [image], [], 'too-large', 5_000_000,
