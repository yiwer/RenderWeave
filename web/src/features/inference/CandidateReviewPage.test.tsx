...
describe('Candidate atomic apply workspace', () => {
  ...
  it('keeps the execution log on the monitor page, out of the review workspace', async () => {
    ...
    expect(await screen.findByLabelText('Candidate 编辑工作区')).toBeTruthy();
    expect(screen.queryByRole('heading', { name: '执行日志' })).toBeNull();
    expect(screen.queryByRole('heading', { name: '阶段与检查点' })).toBeNull();
    expect(api.getInferenceExecutionLogRequest).not.toHaveBeenCalled();
  });
