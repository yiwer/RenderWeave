...
export function problemLabel(code: string) {
  const labels: Record<string, string> = {
    ...
    VISUAL_GROUNDING_READING_ORDER_DUPLICATE: '同级区域阅读序号重复',
    ...
    VISUAL_GROUNDING_REGION_KIND_NORMALIZED: '已按受控别名、唯一结构事实或唯一绑定约束归一化区域类型',
    VISUAL_GROUNDING_REPEATED_ITEM_SLOT_OWNER_NORMALIZED: '已按唯一可见 ITEM 证据归一化重复字段归属',
    VISUAL_GROUNDING_REGION_PARENT_NORMALIZED: '已按唯一最具体既有容器或唯一包含根祖先归一化区域父级',
    VISUAL_GROUNDING_READING_ORDER_NORMALIZED: '已按唯一既有顺序压紧区域阅读序号',
