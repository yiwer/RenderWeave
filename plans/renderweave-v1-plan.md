...
- 当前扩展 Goal：P6/T6-5 图片识别 vNext 为 `in_progress`；N0–N1、N3–N4、N6 已 `automated_verified`，N2 为 `live_verified_mixed_a1_a2`，N5 为 `live_verified_not_promoted`；N7 已完成 pinned Flash、guard v3、Flash/Plus reachability、Plus v14–v22、Max v22 与三模型 v24 A2 smoke。`edc0c28` 的 pipeline 4.9/product-v22 已完成 unique exact-region GROUP-owner support normalization；Plus/Max v22 都完成 live OBSERVE→HIERARCHY→BINDING，但均未命中该 normalization，report 与 stage-gold 质量未达门。`e13bf0c` 的 pipeline 4.10/product-v23 绑定 ephemeral OCR/layout secondary evidence，但 Flash v23 五次仍因 region-kind/parent-kind 固定码停在 OBSERVE。`061101f` 的 pipeline 4.11/product-v24 只对 documented region-kind alias、唯一 exact ITEM parent 与受影响 readingOrder 做 bounded normalization，real-PG、独立 verifier 与 clean server A1 已通过；Flash v24 五次仍停在 OBSERVE，Plus/Max v24 完成三阶段但 slot/binding 均 0 matched，Max 有 16 critical hallucinations、17 blockers，三模型 normalization telemetry 均未命中。全部 ledger CLOSED；相同 v22/v23/v24 不重复，先做零 Provider 的 OBSERVE bounded semantic verifier，不进入 20/60-case final eval。2026-08-11 两次 J1 delta 将三个预算槽位累计 cap 提到 1.5M tokens，单 authorization、attempt/CNY/time 边界不变，且用户已明确允许 Plus，详见 `plans/renderweave-visual-recognition-vnext-plan.md`。
